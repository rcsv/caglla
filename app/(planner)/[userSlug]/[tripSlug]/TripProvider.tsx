 ...
 */
export function TripProvider({ trip: initialTrip, children }: TripProviderProps) {
  const [trip, setTrip] = useState<Trip | null>(initialTrip)
  const [loading, setLoading] = useState(!initialTrip)
  const [error, setError] = useState<'not-found' | 'forbidden' | 'unknown' | null>(null)
    ...
    if (!initialTrip && params?.tripSlug) {
      const fetchTrip = async () => {
        try {
          ...
          logger.debug('TripProvider: Fetching trip from API', { tripSlug: params.tripSlug, hasUser: !!user })
          ...
          if (user) {
            try {
              ...
              logger.debug('TripProvider: Auth failed, falling back to unauthenticated request', authErr)
          ...
          if (response.ok) {
            ...
            logger.debug('TripProvider: Trip fetched successfully', { tripId: tripData.id })
            setTrip(tripData)
            setError(null)
          } else if (response.status === 404) {
            logger.warn('TripProvider: Trip not found', { tripSlug: params.tripSlug })
            setError('not-found')
          } else if (response.status === 403) {
            logger.warn('TripProvider: Access forbidden', { tripSlug: params.tripSlug })
            setError('forbidden')
          } else {
            logger.error('TripProvider: Failed to fetch trip', { status: response.status, tripSlug: params.tripSlug })
            setError('unknown')
          }
        } catch (err) {
