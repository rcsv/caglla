...
export default function useTripPublishing({
		...
		async (startDate: string) => {
			...
			try {
				...
				const response = await makeAuthenticatedRequest(
					`/api/trip/${trip.slug || trip.id}/replica`,
					{
						method: "POST",
	...
	const publish = useCallback(async () => {
		...
		try {
			...
			const refreshedResponse = await makeAuthenticatedRequest(
				`/api/trip/${publishedTrip.id}`,
			);
			...
			const creatorSlug =
				refreshedTrip.creator?.slug || userData?.slug || user.uid;
