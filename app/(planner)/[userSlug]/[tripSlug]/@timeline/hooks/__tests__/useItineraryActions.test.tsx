...
describe("useItineraryActions", () => {
	...
	it("handles schedule addition and selects the itinerary", () => {
		const { result, getLatestTrip, setSelectedItineraryId, updateQuery } =
			setup();
		...
		expect(updated?.days?.[0].itineraries?.map((it) => it.id)).toEqual([
			"it-1",
			"it-3",
			"it-2",
		]);
	...
	it("sends reorder request when moving up", async () => {
		...
		expect(getLatestTrip()?.days?.[0].itineraries?.map((it) => it.id)).toEqual([
			"it-2",
			"it-1",
		]);
	});
