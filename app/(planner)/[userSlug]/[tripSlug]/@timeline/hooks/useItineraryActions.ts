...
export default function useItineraryActions({
  ...
  const findItineraryById = useCallback((id: string): Itinerary | null => {
  ...
  const handleScheduleAdded = useCallback((newItinerary: Itinerary) => {
    if (!trip) return
      ...
      return {
        ...
        days: prevTrip.days?.map(day => {
          if (day.id === newItinerary.day_id) {
            ...
            const sortedItineraries = [...updatedExisting, newItinerary].sort((a, b) => a.sort_number - b.sort_number)
            return {
              ...day,
  ...
  const handleScheduleUpdated = useCallback((updatedItinerary: Itinerary) => {
    if (!trip) return
  ...
  const handleScheduleDelete = useCallback(async (itineraryId: string) => {
    try {
      const response = await makeAuthenticatedRequest(`/api/itineraries/${itineraryId}`, { method: 'DELETE' })
  ...
  const reorderRequest = useCallback(async (dayId: string, updates: Itinerary[]) => {
    const response = await makeAuthenticatedRequest('/api/itineraries/reorder', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ dayId, itineraryIds: updates.map(item => item.id) }),
    })
    return response
  }, [])

  const handleMoveUp = useCallback(async (itineraryId: string, dayId: string) => {
    if (!trip) return
  ...
  const handleMoveDown = useCallback(async (itineraryId: string, dayId: string) => {
    if (!trip) return
    ...
    const sortedItineraries = [...day.itineraries].sort((a, b) => a.sort_number - b.sort_number)
    const currentIndex = sortedItineraries.findIndex(item => item.id === itineraryId)
    if (currentIndex >= sortedItineraries.length - 1 || currentIndex === -1) return
  ...
  const handleMoveToDay = useCallback(async (itineraryId: string, targetDayId: string) => {
    if (!trip) return
  ...
  const handleReorderItineraries = useCallback(async (dayId: string, reorderedItineraries: Itinerary[]) => {
    if (!trip) return
