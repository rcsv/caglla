...
const FRIEND_ACTIVITIES = [
  ...
  },
  {
    ...
    cover: 'https://images.unsplash.com/photo-1545569341-9eb8b30979d6?auto=format&fit=crop&w=1200&q=80',
    tags: ['#グルメ', '#町家ステイ', '#2泊3日'],
  ...
  },
  {
    ...
    title: '子連れで巡る北海道ドライブ',
    location: '札幌 / 富良野 / 美瑛',
    summary: '未就学児2人との北海道ドライブ。動物園やファーム富田を効率的に巡る実例。',
    cover: 'https://images.unsplash.com/photo-1473625247510-8ceb1760943f?auto=format&fit=crop&w=1200&q=80',
    tags: ['#家族旅', '#ドライブ', '#夏休み'],
...
const PLAN_IDEAS = [
  {
    ...
    cover: 'https://images.unsplash.com/photo-1500048993953-d23a436266cf?auto=format&fit=crop&w=1200&q=80',
    creator: { name: 'Lina Planner', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Lina', role: 'Travel Creator' },
  ...
  },
  {
    ...
    summary: '屋台よりも糸島の静かなカフェとワーケーションスポットを重視した構成。',
    cover: 'https://images.unsplash.com/photo-1469474968028-56623f02e42e?auto=format&fit=crop&w=1200&q=80',
    creator: { name: 'Takuya Nomad', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Takuya', role: 'Nomad Planner' },
  ...
  },
  {
    ...
    theme: 'リモートワーク',
    tags: ['ウェルネス', 'ヨガ', 'サーフィン'],
    summary: '朝ヨガと夕方サーフで1日を区切るワークデイ設計。ビザ・SIM情報付き。',
    cover: 'https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=1200&q=80',
    creator: { name: 'Studio Paon', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Paon', role: 'Boutique Planner' },
  ...
  },
  {
    ...
    theme: '親子旅',
    tags: ['動物体験', '街歩き', '海沿い'],
    summary: 'ライトレールとフェリーで巡る親子旅。タロンガ動物園とボンダイキッズプログラム込み。',
    cover: 'https://images.unsplash.com/photo-1506976785307-8732e854ad03?auto=format&fit=crop&w=1200&q=80',
    creator: { name: 'Haruka & Co.', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Haruka', role: 'Family Travel Lab' },
...
export default function HomeV2Page() {
  ...
  const { trips, tripsLoading, planConfig, userData, userDataLoading } = useUserData()
                ...
                {TAB_OPTIONS.map((tab) => (
                  ...
                  >
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white shadow-inner">
                      <Icon icon={tab.icon} className={activeTab === tab.id ? 'text-indigo-600' : 'text-slate-400'} />
                    </div>
                    <div>
            ...
            <section className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Link href="/plan" className="text-xs text-indigo-600 hover:text-indigo-800 flex items-center gap-1">
                    すべて
              ...
              {activeTrips.length === 0 ? (
                <div className="text-center py-4 text-gray-500">
                  ...
                  <Link href="/plan" className="inline-block text-indigo-600 hover:text-indigo-800 text-xs">
                    新規作成
                  </Link>
                </div>
              ) : (
                          ...
                          <div className="md:flex">
                            ...
                            <div className="md:w-2/3 p-4 flex flex-col gap-3">
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  <h3 className="text-base font-semibold text-gray-900 line-clamp-2">{trip.title || 'Untitled Trip'}</h3>
              ...
              {upcomingTrips.length === 0 ? (
                ...
                <div className="space-y-2">
                  {upcomingTrips.map((trip, index) => {
                    ...
                    const startDate = toDateOrNull(trip.start_date)
                    const daysUntil = startDate ? Math.ceil((startDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)) : null
                        ...
                        <div className="border border-gray-200 rounded-lg overflow-hidden hover:border-emerald-300 hover:shadow-sm transition-all">
                          <div className="flex gap-3">
                            ...
                            <div className="py-2 pr-3 flex-1">
                              ...
                              {startDate && (
                                <div className="flex items-center gap-2">
                                  <p className="text-xs text-gray-500">{dateUtils.formatDateRange(trip.start_date!, trip.end_date!)}</p>
  ...
  return (
    <section className="space-y-6">
      {FRIEND_ACTIVITIES.map((activity) => {
        ...
        const typeLabel = activity.type === 'shared' ? 'SHARED TRIP' : 'PLAN TEMPLATE'
              ...
              <div className="flex-1">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  <span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${typeStyles}`}>
                    <Icon icon={activity.type === 'shared' ? 'mdi:share-variant' : 'mdi:file-document-outline'} className="h-3.5 w-3.5" />
                    {typeLabel}
                  </span>
                  <span className="text-xs text-slate-400">{activity.userHandle}</span>
                </div>

                <h2 className="mt-4 text-xl font-semibold text-slate-900">{activity.title}</h2>
                ...
                <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-500">
                  {activity.tags.map((tag) => (
                    <span key={`${activity.id}-${tag}`} className="rounded-full bg-slate-100 px-3 py-1">
                      {tag}
                    </span>
                  ))}
                </div>
        ...
        <article key={plan.id} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
          ...
          <div className="space-y-4 p-5">
            ...
            <div className="flex flex-wrap gap-2 text-xs text-slate-500">
              {plan.tags.map((tag) => (
                <span key={`${plan.id}-${tag}`} className="rounded-full bg-slate-100 px-3 py-1">
                  #{tag}
                </span>
              ))}
            </div>
          ...
          <div className="flex-1 space-y-3">
            ...
            <div className="flex flex-wrap gap-2 text-xs text-slate-500">
              {trip.highlights.map((item) => (
                <span key={`${trip.id}-${item}`} className="rounded-full bg-slate-100 px-3 py-1">
                  {item}
                </span>
              ))}
            </div>
          ...
          <div className="w-full md:w-64">
            <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 text-sm text-slate-600">
              <div className="flex items-center justify-between text-slate-500">
                <span>公開期限</span>
                <span className="font-semibold text-slate-900">{trip.expires}</span>
              </div>
...
function VisibilityBadge({ label }: { label: string }) {
  ...
  if (label.includes('全体')) style = 'bg-emerald-50 text-emerald-700 border border-emerald-100'
  if (label.includes('フォロワー')) style = 'bg-blue-50 text-blue-700 border border-blue-100'
  if (label.includes('リンク')) style = 'bg-amber-50 text-amber-700 border border-amber-100'
