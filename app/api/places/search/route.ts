...
import {
	...
	parseApiResponse,
} from "@/lib/api/external-api-helpers";

// 新Places API (v1) のエンドポイント
const GOOGLE_PLACES_API_URL_NEW =
...
export const POST = composeMiddleware(
	...
	try {
		...
		logger.debug("Searching for place with new Places API v1", {
			query,
			...
			async () => {
				const response = await fetch(GOOGLE_PLACES_API_URL_NEW, {
					...
					headers: {
						...
						"X-Goog-FieldMask":
							"places.id,places.displayName,places.formattedAddress,places.location,places.types,places.rating,places.userRatingCount,places.priceLevel,places.photos",
					},
					body: JSON.stringify({
						textQuery: query,
