...
export const POST = composeMiddleware(
	...
	const finalCostCurrency =
		cost_currency || tripData?.default_currency || undefined;
	...
	try {
		...
		logger.warn("Failed to increment trip.stats.itineraries", {
			tripId,
			error: e,
		});
	}
			...
			try {
				...
				if (resp.ok) {
					...
					if (result.address_components)
						cachePayload.address_components = result.address_components;
					if (result.photos)
						cachePayload.photos = result.photos.map((p: any) => ({
					...
					if (result.user_ratings_total !== undefined)
						cachePayload.user_ratings_total = result.user_ratings_total;
					if (result.price_level !== undefined)
						cachePayload.price_level = result.price_level;
					...
					if (result.editorial_summary)
						cachePayload.editorial_summary = result.editorial_summary;
					...
					await adminDb
						...
						.set(cachePayload);
					logger.debug(
						"Successfully saved to PlacesCache from API (NEW FORMAT)",
						{ cacheKey },
					);
