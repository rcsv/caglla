		...
		if (body.day_id !== undefined && body.sort_number !== undefined) {
			...
			if (reservation !== undefined) {
				...
				if (reservation?.start_date)
					updateData.reservation_start_ts = reservation.start_date;
				if (reservation?.end_date)
					updateData.reservation_end_ts = reservation.end_date;
				if (reservation?.departure_at)
					updateData.reservation_start_ts = reservation.departure_at;
				if (reservation?.arrival_at)
					updateData.reservation_end_ts = reservation.arrival_at;
				...
				if (reservation) {
					...
					updateData.reservation_summary = generateReservationSummary(
						reservation,
						placeName,
					);
				}
...
export async function DELETE(
	...
	try {
		...
		try {
			...
			logger.warn("Failed to decrement trip.stats.itineraries on delete", {
				itineraryId: id,
				error: e,
			});
		}
		...
		if (dayId && deletedSortNumber > 0) {
			...
			if (!subsequentItineraries.empty) {
				...
				await batch.commit();
				logger.debug("Renumbered subsequent itineraries", {
					count: subsequentItineraries.docs.length,
				});
			}
		}
