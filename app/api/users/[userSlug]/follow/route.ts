...
export async function DELETE(
	...
	try {
		...
		if (error instanceof Error) {
			...
			if (
				error.message.includes("Not following this user") ||
				error.message.includes("Not following")
			) {
				return notFound("Not following this user");
			}
			if (
				error.message.includes("Only follower can unfollow") ||
				error.message.includes("follower")
			) {
				// 403 Forbidden は標準的なエラーレスポンスなので、そのまま返す
				return NextResponse.json(
					{ error: "Only follower can unfollow" },
					{ status: 403 },
				);
			}
			if (error.message.includes("User not found")) {
