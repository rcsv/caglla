...
export const PUT = composeMiddleware(
	...
	const { name, profile_image_url, bio, gender, preferences } = body;
	...
	const existingUser =
		await adminUserOperations.getUserByAuthUid(authenticatedUserId);
	if (!existingUser) {
		return notFound("User");
	}
		...
		const existingSlugs = usersSnapshot.docs
			...
			.filter(
				(value): value is string =>
					Boolean(value) && value !== existingUser.slug,
			);
