		...
		const items = docs.map((doc: FirebaseFirestore.QueryDocumentSnapshot) => {
			...
			return {
				...
				reservation_start_ts:
					toDateOrNull(data.reservation_start_ts)?.toISOString() ||
					data.reservation_start_ts,
				reservation_end_ts:
					toDateOrNull(data.reservation_end_ts)?.toISOString() ||
					data.reservation_end_ts,
