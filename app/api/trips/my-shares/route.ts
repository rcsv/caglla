...
import {
	getUserTripsWithBackwardCompatibility,
	encodeCursor,
} from "@/lib/firebase/trip-query-helpers";
...
export const GET = authApi(
	async (
		request: NextRequest,
		ctx,
	): Promise<NextResponse<MySharesResponse>> => {
		...
		const templateFilter =
			(searchParams.get("template") as "include" | "only" | "exclude" | null) ??
			"exclude";
		...
		try {
			...
			let filteredTrips = trips.filter(
				(trip) =>
					trip.access_level === "public" || trip.access_level === "unlisted",
			);

			// テンプレートフィルタ（デフォルト: exclude）
			if (templateFilter === "exclude") {
				filteredTrips = filteredTrips.filter(
					(trip) => trip.is_template !== true,
				);
				...
				return NextResponse.json(
					{
						trips: [],
						error:
							"Firestore index required. Please create a composite index for trips collection: user_id (ascending) and updated_at (descending)",
