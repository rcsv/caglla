...
export const GET = authApi(async (request: NextRequest, ctx) => {
	...
	if (!adminDb) {
		logger.warn(
			"Firebase Admin SDK not initialized, returning empty recommended trips",
		);
	...
	const uniqueUserIds = Array.from(
		new Set(trips.map((t) => t.user_id).filter(Boolean)),
	) as string[];
	...
	for (let i = 0; i < uniqueUserIds.length; i += 10) {
		...
		try {
			...
			logger.error("Failed to batch fetch creators", error, {
