...
export const GET = authApi(async (request: NextRequest, ctx) => {
	...
	if (includeShared) {
		...
		const sharedTripUsers =
			await adminTripUserOperations.getTripsByUserId(userId);
		...
		const validSharedTrips = sharedTrips.filter(
	...
	const tripsWithDetails = await Promise.all(
		trips.map(async (trip): Promise<TripWithDestination> => {
			...
			try {
				...
				logger.error("Error fetching creator for trip", error, {
					tripId: trip.id,
				});
			}
			...
			try {
				if (!destinationPlace && trip.destination_place_id) {
					...
					const lang = "en";
					destinationPlace =
						(await resolveDestinationPlace(trip.destination_place_id, lang)) ||
						undefined;
					...
					if (destinationPlace) {
						...
						console.log(
							"❌ Places Cache not found for any language:",
							trip.destination_place_id,
						);
					}
				}
			} catch (error) {
				console.error("❌ Error resolving destination_place for trip:", error, {
					tripId: trip.id,
				});
