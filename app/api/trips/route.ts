	...
	const tripsWithDetails = await Promise.all(
		trips.map(async (trip): Promise<TripWithDestination> => {
			...
			try {
				...
				logger.error("Error resolving destination_place for trip", error, {
					tripId: trip.id,
				});
			}
...
export const POST = composeMiddleware(
	...
	const requestedAccessLevel =
		body.accessLevel ?? body.access_level ?? "private";
	...
	const userDocumentId = user.id;
	logger.debug("Resolved user document ID", {
		authUid: userId,
		userDocumentId,
	});
	...
	const existingTrips =
		await adminTripOperations.getTripsByUserId(userDocumentId);
	...
	const isAvatarPath =
		...
		((imageUrl.includes("/users/") && imageUrl.includes("/avatar/")) ||
			(decodeURIComponent(imageUrl).includes("/users/") &&
				decodeURIComponent(imageUrl).includes("/avatar/")));
	if (isAvatarPath) {
		logger.info(
			"Image URL is in avatar path, will move to trip path after creation",
			{ imageUrl, userId },
		);
	...
	const resolvedDestPlaceId: string | undefined =
	...
	const shouldMoveImage =
		finalImageUrl &&
		((finalImageUrl.includes("/users/") &&
			finalImageUrl.includes("/avatar/")) ||
			(decodeURIComponent(finalImageUrl).includes("/users/") &&
				decodeURIComponent(finalImageUrl).includes("/avatar/")));
	if (shouldMoveImage) {
		try {
			...
			const newImageUrl = await adminTripOperations.moveImageToTripPath(
				finalImageUrl,
				trip.id,
			);
			...
			if (cacheDoc.exists) {
				...
				await cacheDoc.ref
					.update({
						...
						access_count: (cacheDoc.data().access_count || 0) + 1,
					})
					.catch(() => {});
				...
				try {
					...
					if (pd.address_components)
						cachePayload.address_components = pd.address_components;
					if (pd.photos) cachePayload.photos = pd.photos;
					if (pd.rating !== undefined) cachePayload.rating = pd.rating;
					if (pd.user_ratings_total !== undefined)
						cachePayload.user_ratings_total = pd.user_ratings_total;
					if (pd.price_level !== undefined)
						cachePayload.price_level = pd.price_level;
					if (pd.types) cachePayload.types = pd.types;
					if (pd.opening_hours?.weekday_text)
						cachePayload.opening_hours = {
							weekday_text: pd.opening_hours.weekday_text,
						};
					if (pd.international_phone_number)
						cachePayload.international_phone_number =
							pd.international_phone_number;
					if (pd.website) cachePayload.website = pd.website;
					if (pd.editorial_summary)
						cachePayload.editorial_summary = pd.editorial_summary;
					...
					logger.error(
						"Failed to save to PlacesCache (NEW FORMAT):",
						cacheError,
					);
					// キャッシュ保存失敗は致命的ではない
				}
