	...
	try {
		...
		for (const doc of tripsSnapshot.docs) {
			...
			if (dest?.place_id && !data.destination_place_id) {
				...
				if (dest.address_components)
					cachePayload.address_components = dest.address_components;
				if (dest.photos) cachePayload.photos = dest.photos;
				if (dest.rating !== undefined) cachePayload.rating = dest.rating;
				if (dest.user_ratings_total !== undefined)
					cachePayload.user_ratings_total = dest.user_ratings_total;
				if (dest.price_level !== undefined)
					cachePayload.price_level = dest.price_level;
				if (dest.types) cachePayload.types = dest.types;
				if (dest.opening_hours?.weekday_text)
					cachePayload.opening_hours = {
						weekday_text: dest.opening_hours.weekday_text,
					};
				if (dest.international_phone_number)
					cachePayload.international_phone_number =
						dest.international_phone_number;
				if (dest.website) cachePayload.website = dest.website;
				if (dest.editorial_summary)
					cachePayload.editorial_summary = dest.editorial_summary;
				await adminDb
