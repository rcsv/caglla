...
async function handleFollowUser(request: NextRequest, userSlug: string): Promise<Response> {
  return await followUser(request, { params: Promise.resolve({ userSlug }) })
}

async function handleUnfollowUser(request: NextRequest, userSlug: string): Promise<Response> {
  return await unfollowUser(request, { params: Promise.resolve({ userSlug }) })
}

async function handleGetFollowState(request: NextRequest, userSlug: string): Promise<Response> {
  return await getFollowState(request, { params: Promise.resolve({ userSlug }) })
}
...
describe('User Follow API Routes', () => {
  ...
  describe('POST /api/users/[userSlug]/follow', () => {
    it('should allow authenticated users to follow other users', async () => {
      const request = new NextRequest(`http://localhost/api/users/${otherUserSlug}/follow`, {
        method: 'POST',
    ...
    it('should deny following self', async () => {
      const request = new NextRequest(`http://localhost/api/users/${userSlug}/follow`, {
        method: 'POST',
        headers: createAuthHeader(userId),
      })
    ...
    it('should deny duplicate follow requests', async () => {
      // 最初のフォロー
      const request1 = new NextRequest(`http://localhost/api/users/${otherUserSlug}/follow`, {
        method: 'POST',
        headers: createAuthHeader(userId),
      })
      ...
      const request2 = new NextRequest(`http://localhost/api/users/${otherUserSlug}/follow`, {
        method: 'POST',
        headers: createAuthHeader(userId),
      })
  ...
  describe('DELETE /api/users/[userSlug]/follow', () => {
    ...
    it('should allow user to unfollow another user', async () => {
      ...
      // フォロー関係が削除されたことを確認
      const followRef = db.collection(COLLECTIONS.USER_FOLLOWS).doc(`${userId}_${otherUserId}`)
      const followDoc = await followRef.get()
