...
import { createMockTrip, createMockDay, createMockItinerary } from '@/lib/__tests__/helpers/test-data'
...
import { GET as getItineraries, POST as postItineraries } from '@/app/api/itineraries/route'
import { PUT as putItinerary, DELETE as deleteItinerary } from '@/app/api/itineraries/[id]/route'
...
async function handlePutItinerary(request: NextRequest, itineraryId: string): Promise<Response> {
  return await putItinerary(request, { params: Promise.resolve({ id: itineraryId }) })
}

async function handleDeleteItinerary(request: NextRequest, itineraryId: string): Promise<Response> {
  return await deleteItinerary(request, { params: Promise.resolve({ id: itineraryId }) })
}
...
async function handleDuplicateItinerary(request: NextRequest): Promise<Response> {
  return await duplicateItinerary(request)
}

async function handleReorderItineraries(request: NextRequest): Promise<Response> {
...
describe('Itineraries API Routes', () => {
  ...
  beforeEach(async () => {
    ...
    const itinerariesSnapshot = await db.collection(COLLECTIONS.ITINERARIES).get()
    ...
    await db.collection(COLLECTIONS.ITINERARIES).doc(itinerary.id).set(itinerary)
  })

  describe('GET /api/itineraries?day_id=xxx', () => {
    it('should return itineraries for authenticated owner', async () => {
      const request = new NextRequest(`http://localhost/api/itineraries?day_id=${day.id}`, {
        method: 'GET',
    ...
    it('should deny access to other users days', async () => {
      const request = new NextRequest(`http://localhost/api/itineraries?day_id=${otherDay.id}`, {
        method: 'GET',
    ...
    it('should return 404 for non-existent day', async () => {
      const request = new NextRequest('http://localhost/api/itineraries?day_id=non-existent-day', {
        method: 'GET',
        headers: createAuthHeader(userId),
      })
    ...
    it('should return empty array for day with no itineraries', async () => {
      ...
      const request = new NextRequest(`http://localhost/api/itineraries?day_id=${emptyDay.id}`, {
        method: 'GET',
        headers: createAuthHeader(userId),
      })
  ...
  describe('POST /api/itineraries', () => {
    it('should create itinerary for authenticated owner', async () => {
      ...
      const itineraryDoc = await db.collection(COLLECTIONS.ITINERARIES).doc(data.id).get()
  ...
  describe('PUT /api/itineraries/[id]', () => {
    ...
    it('should update itinerary (no auth required currently)', async () => {
      const request = new NextRequest(`http://localhost/api/itineraries/${itinerary.id}`, {
        method: 'PUT',
        headers: {},
        body: JSON.stringify({
          title: 'Updated Title',
      ...
      const itineraryDoc = await db.collection(COLLECTIONS.ITINERARIES).doc(itinerary.id).get()
    ...
    it('should return 404 for non-existent itinerary', async () => {
      const request = new NextRequest('http://localhost/api/itineraries/non-existent-id', {
        method: 'PUT',
        headers: {},
        body: JSON.stringify({
          title: 'Test Title',
  ...
  describe('DELETE /api/itineraries/[id]', () => {
    ...
    it('should renumber subsequent itineraries after deletion', async () => {
      ...
      await db.collection(COLLECTIONS.ITINERARIES).doc(itinerary2.id).set(itinerary2)
      await db.collection(COLLECTIONS.ITINERARIES).doc(itinerary3.id).set(itinerary3)

      // sort_number=2のitineraryを削除
      const request = new NextRequest(`http://localhost/api/itineraries/${itinerary2.id}`, {
        method: 'DELETE',
        headers: {},
      })
      ...
      const itinerary3Doc = await db.collection(COLLECTIONS.ITINERARIES).doc(itinerary3.id).get()
  ...
  describe('POST /api/itineraries/insert', () => {
    it('should insert itinerary at specified position', async () => {
      const request = new NextRequest('http://localhost/api/itineraries/insert', {
        method: 'POST',
        headers: createAuthHeader(userId),
        body: JSON.stringify({
          day_id: day.id,
      ...
      const existingItineraryDoc = await db.collection(COLLECTIONS.ITINERARIES).doc(itinerary.id).get()
  ...
  describe('POST /api/itineraries/move-to-day', () => {
    ...
    it('should move itinerary to different day (no auth required currently)', async () => {
      ...
      const request = new NextRequest('http://localhost/api/itineraries/move-to-day', {
        method: 'PUT',
        headers: {},
        body: JSON.stringify({
  ...
  describe('POST /api/itineraries/duplicate-to-day', () => {
    ...
    it('should duplicate itinerary to different day (no auth required currently)', async () => {
      ...
      const request = new NextRequest('http://localhost/api/itineraries/duplicate-to-day', {
        method: 'POST',
        headers: {},
        body: JSON.stringify({
          itinerary_id: itinerary.id,
      ...
      const duplicateDoc = await db.collection(COLLECTIONS.ITINERARIES).doc(data.id).get()
  ...
  describe('POST /api/itineraries/reorder', () => {
    ...
    it('should reorder itineraries (no auth required currently)', async () => {
      ...
      // 並び替え: 3, 1, 2の順にする
      const request = new NextRequest('http://localhost/api/itineraries/reorder', {
        method: 'POST',
        headers: {},
        body: JSON.stringify({
          dayId: day.id,
      ...
      const itinerary1Doc = await db.collection(COLLECTIONS.ITINERARIES).doc(itinerary.id).get()
      const updatedItinerary1 = itinerary1Doc.data() as Itinerary
      expect(updatedItinerary1.sort_number).toBe(2)

      const itinerary2Doc = await db.collection(COLLECTIONS.ITINERARIES).doc(itinerary2.id).get()
