...
import {
	GET as getTripLikes,
	POST as likeTrip,
	DELETE as unlikeTrip,
} from "@/app/api/trip/[tripSlug]/likes/route";

// ヘルパー関数：API Routesを呼び出す
async function handleLikeTrip(
	request: NextRequest,
	tripSlug: string,
): Promise<Response> {
...
async function handleUnlikeTrip(
	request: NextRequest,
	tripSlug: string,
): Promise<Response> {
...
async function handleGetTripLikes(
	request: NextRequest,
	tripSlug: string,
): Promise<Response> {
	...
	describe("POST /api/trip/[tripSlug]/likes", () => {
		it("should allow authenticated users to like public trips", async () => {
			const request = new NextRequest(
				"http://localhost/api/trip/public-trip-1/likes",
				{
					method: "POST",
		...
		it("should deny liking private trips", async () => {
			const request = new NextRequest(
				"http://localhost/api/trip/private-trip-1/likes",
				{
					method: "POST",
					headers: createAuthHeader(userId),
				},
