...
import {
	GET as getUserBySlug,
	PUT as putUserBySlug,
} from "@/app/api/users/[userSlug]/route";
import { POST as checkSlug } from "@/app/api/users/check-slug/route";
import {
	GET as getUserPlan,
	PUT as putUserPlan,
} from "@/app/api/user/plan/route";
...
async function handleGetUserBySlug(
	...
	return await getUserBySlug(request, {
		params: Promise.resolve({ userSlug }),
	});
}

async function handlePutUserBySlug(
	request: NextRequest,
	userSlug: string,
): Promise<Response> {
	return await putUserBySlug(request, {
		params: Promise.resolve({ userSlug }),
	});
}
...
describe("Users API Routes", () => {
	...
	describe("GET /api/users/[userSlug]", () => {
		...
		it("should return 404 for non-existent user slug", async () => {
			const request = new NextRequest(
				"http://localhost/api/users/non-existent-slug",
				{
					method: "GET",
					headers: createUnauthenticatedHeader(),
				},
	...
	describe("PUT /api/users/[userSlug]", () => {
		...
		it("should deny updating other users", async () => {
			const request = new NextRequest(
				`http://localhost/api/users/${otherUser.slug}`,
				{
					method: "PUT",
