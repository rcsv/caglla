...
async function handleCreateComment(
  ...
  return await createComment(request, { params: Promise.resolve({ tripSlug }) })
}
...
async function handleGetComments(request: NextRequest, tripSlug: string): Promise<Response> {
  return await getTripComments(request, { params: Promise.resolve({ tripSlug }) })
}
...
describe('Trip Comments API Routes', () => {
  ...
  describe('POST /api/trip/[tripSlug]/comments', () => {
    it('should allow authenticated users to create comments on public trips', async () => {
      const request = new NextRequest('http://localhost/api/trip/public-trip-1/comments', {
        method: 'POST',
    ...
    it('should allow creating nested comments', async () => {
      // 親コメントを作成
      const parentRequest = new NextRequest('http://localhost/api/trip/public-trip-1/comments', {
        method: 'POST',
        headers: createAuthHeader(userId),
        body: JSON.stringify({
      ...
      const parentResponse = await handleCreateComment(parentRequest, 'public-trip-1')
      const parentData = await parentResponse.json()

      // ネストコメントを作成
      const nestedRequest = new NextRequest('http://localhost/api/trip/public-trip-1/comments', {
        method: 'POST',
      ...
      const nestedResponse = await handleCreateComment(nestedRequest, 'public-trip-1')
    ...
    it('should deny commenting on private trips', async () => {
      const request = new NextRequest('http://localhost/api/trip/private-trip-1/comments', {
        method: 'POST',
        headers: createAuthHeader(userId),
        body: JSON.stringify({
  ...
  describe('PUT /api/trip/[tripSlug]/comments/[commentId]', () => {
    ...
    it('should allow comment owner to update comment', async () => {
      ...
      const response = await handleUpdateComment(request, 'public-trip-1', commentId)
      const data = await response.json()
  ...
  describe('DELETE /api/trip/[tripSlug]/comments/[commentId]', () => {
    ...
    it('should allow comment owner to delete comment', async () => {
      ...
      const response = await handleDeleteComment(request, 'public-trip-1', commentId)
  ...
  describe('GET /api/trip/[tripSlug]/comments', () => {
    beforeEach(async () => {
      // 複数のコメントを作成
      const request1 = new NextRequest('http://localhost/api/trip/public-trip-1/comments', {
        method: 'POST',
        headers: createAuthHeader(userId),
        body: JSON.stringify({
          content: 'Comment 1',
      ...
      const request2 = new NextRequest('http://localhost/api/trip/public-trip-1/comments', {
        method: 'POST',
        headers: createAuthHeader(otherUserId),
        body: JSON.stringify({
          content: 'Comment 2',
      ...
      // 削除されたコメントも作成
      const request3 = new NextRequest('http://localhost/api/trip/public-trip-1/comments', {
        method: 'POST',
        headers: createAuthHeader(userId),
        body: JSON.stringify({
    ...
    it('should return comments for public trips (excluding deleted)', async () => {
      ...
      expect(data.some((c: TripComment) => c.content === 'Comment 1')).toBe(true)
      expect(data.some((c: TripComment) => c.content === 'Comment 2')).toBe(true)
    })
