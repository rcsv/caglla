...
describe("Feed API Routes", () => {
	...
	describe("GET /api/feed/public", () => {
		it("should return only public trips", async () => {
			...
			expect(
				data.trips.every((trip: Trip) => trip.access_level === "public"),
			).toBe(true);
			expect(data.trips.some((trip: Trip) => trip.id === publicTrip1.id)).toBe(
				true,
			);
			expect(data.trips.some((trip: Trip) => trip.id === publicTrip2.id)).toBe(
				true,
			);
			expect(data.trips.some((trip: Trip) => trip.id === publicTrip3.id)).toBe(
				true,
			);
		});
		...
		it("should exclude private trips", async () => {
			...
			expect(response.status).toBe(200);
			expect(data.trips.some((trip: Trip) => trip.id === privateTrip.id)).toBe(
				false,
			);
		});
		...
		it("should respect limit query parameter", async () => {
			const request = new NextRequest(
				"http://localhost/api/feed/public?limit=2",
				{
					method: "GET",
					headers: createAuthHeader(userId),
				},
	...
	describe("GET /api/feed/trending", () => {
		...
		it("should exclude trips without trending_score", async () => {
			...
			await db
				...
				.doc(tripWithoutScore.id)
				.set(tripWithoutScore);
			...
			expect(response.status).toBe(200);
			expect(
				data.trips.some((trip: Trip) => trip.id === tripWithoutScore.id),
			).toBe(false);
		});

		it("should respect limit query parameter", async () => {
			const request = new NextRequest(
				"http://localhost/api/feed/trending?limit=2",
				{
					method: "GET",
					headers: createAuthHeader(userId),
				},
	...
	describe("GET /api/feed/following", () => {
		...
		it("should return only public trips from followed users", async () => {
			...
			expect(
				data.trips.every((trip: Trip) => trip.user_id === otherUserId),
			).toBe(true);
		});
		...
		it("should exclude trips from non-followed users", async () => {
			...
			await db
				.collection(COLLECTIONS.TRIPS)
				.doc(tripFromNonFollowed.id)
				.set(tripFromNonFollowed);
			...
			expect(response.status).toBe(200);
			expect(
				data.trips.some((trip: Trip) => trip.id === tripFromNonFollowed.id),
			).toBe(false);
		});
		...
		it("should respect limit query parameter", async () => {
			const request = new NextRequest(
				"http://localhost/api/feed/following?limit=2",
				{
					method: "GET",
					headers: createAuthHeader(userId),
				},
