...
describe('Trip Templates API Routes', () => {
  ...
  beforeEach(async () => {
    ...
    await db.collection(COLLECTIONS.TRIPS).doc(templateTrip1.id).set(templateTrip1)
    await db.collection(COLLECTIONS.TRIPS).doc(templateTrip2.id).set(templateTrip2)
    await db.collection(COLLECTIONS.TRIPS).doc(publicNonTemplateTrip.id).set(publicNonTemplateTrip)
    await db.collection(COLLECTIONS.TRIPS).doc(privateTemplateTrip.id).set(privateTemplateTrip)
  })

  describe('GET /api/trips/templates', () => {
    it('should return public template trips only', async () => {
      const request = new NextRequest('http://localhost:3000/api/trips/templates')
      ...
      expect(data.trips.map((t: Trip) => t.id)).not.toContain('public-non-template-1')
      expect(data.trips.map((t: Trip) => t.id)).not.toContain('private-template-1')
      expect(data.trips.every((t: Trip) => t.is_template === true)).toBe(true)
      expect(data.trips.every((t: Trip) => t.access_level === 'public')).toBe(true)
    })
    ...
    it('should exclude user trips when excludeMyTrips=true and user is authenticated', async () => {
      ...
      await db.collection(COLLECTIONS.TRIPS).doc(myTemplateTrip.id).set(myTemplateTrip)
      ...
      expect(response.status).toBe(200)
      expect(data.trips).toHaveLength(2)
      expect(data.trips.map((t: Trip) => t.id)).not.toContain('my-template-trip-1')
      expect(data.trips.every((t: Trip) => t.user_id !== userId)).toBe(true)
    })

    it('should respect limit parameter', async () => {
      const request = new NextRequest('http://localhost:3000/api/trips/templates?limit=1')
      const response = await handleGetTemplateTrips(request)
      const data = await response.json()
    ...
    it('should support pagination with cursor', async () => {
      const firstRequest = new NextRequest('http://localhost:3000/api/trips/templates?limit=1')
    ...
    it('should return 400 for invalid limit parameter', async () => {
      const request = new NextRequest('http://localhost:3000/api/trips/templates?limit=200')
      const response = await handleGetTemplateTrips(request)
  ...
  describe('GET /api/trips/my-shares', () => {
    beforeEach(async () => {
      ...
      await db.collection(COLLECTIONS.TRIPS).doc(myPublicTrip1.id).set(myPublicTrip1)
      await db.collection(COLLECTIONS.TRIPS).doc(myPublicTrip2.id).set(myPublicTrip2)
      await db.collection(COLLECTIONS.TRIPS).doc(myPrivateTrip.id).set(myPrivateTrip)
    })

    it('should return only user\'s public trips', async () => {
      const request = new NextRequest('http://localhost:3000/api/trips/my-shares', {
        headers: createAuthHeader(userId),
      })
      ...
      expect(data.trips.map((t: Trip) => t.id)).not.toContain('my-private-trip-1')
      expect(data.trips.every((t: Trip) => t.user_id === userId)).toBe(true)
      expect(data.trips.every((t: Trip) => t.access_level === 'public')).toBe(true)
    })
    ...
    it('should not return other users\' public trips', async () => {
      ...
      expect(data.trips.map((t: Trip) => t.id)).not.toContain('template-trip-1')
      expect(data.trips.map((t: Trip) => t.id)).not.toContain('template-trip-2')
      expect(data.trips.map((t: Trip) => t.id)).not.toContain('public-non-template-1')
    })
    ...
    it('should respect limit parameter', async () => {
      const request = new NextRequest('http://localhost:3000/api/trips/my-shares?limit=1', {
        headers: createAuthHeader(userId),
      })
      const response = await handleGetMySharedTrips(request)
      const data = await response.json()
    ...
    it('should support pagination with cursor', async () => {
      const firstRequest = new NextRequest('http://localhost:3000/api/trips/my-shares?limit=1', {
        headers: createAuthHeader(userId),
      })
    ...
    it('should return 400 for invalid limit parameter', async () => {
      const request = new NextRequest('http://localhost:3000/api/trips/my-shares?limit=200', {
        headers: createAuthHeader(userId),
      })
      const response = await handleGetMySharedTrips(request)
