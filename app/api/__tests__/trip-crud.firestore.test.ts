...
} from "@/app/api/trip/[tripSlug]/route";
import {
	POST as publishTrip,
	DELETE as unpublishTrip,
} from "@/app/api/trip/[tripSlug]/publish/route";
...
async function handleUnpublishTrip(
	...
	return await unpublishTrip(request, {
		params: Promise.resolve({ tripSlug }),
	});
}

async function handleReplicateTrip(
	request: NextRequest,
	tripSlug: string,
): Promise<Response> {
	return await replicateTrip(request, {
		params: Promise.resolve({ tripSlug }),
	});
}
...
describe("Trip CRUD API Routes", () => {
	...
	describe("GET /api/trip/[tripSlug]", () => {
		it("should return public trip for any user", async () => {
			const request = new NextRequest(
				`http://localhost/api/trip/${publicTrip.slug}`,
				{
					method: "GET",
		...
		it("should return private trip for owner", async () => {
			const request = new NextRequest(
				`http://localhost/api/trip/${privateTrip.slug}`,
				{
					method: "GET",
		...
		it("should return 404 for non-existent trip", async () => {
			const request = new NextRequest(
				"http://localhost/api/trip/non-existent-trip",
				{
					method: "GET",
					headers: createUnauthenticatedHeader(),
				},
	...
	describe("POST /api/trip/[tripSlug]/publish", () => {
		it("should publish private trip for authenticated owner", async () => {
			const request = new NextRequest(
				`http://localhost/api/trip/${privateTrip.slug}/publish`,
				{
					method: "POST",
					headers: createAuthHeader(userId),
					body: JSON.stringify({}),
		...
		it("should return 404 for non-existent trip", async () => {
			const request = new NextRequest(
				"http://localhost/api/trip/non-existent-trip/publish",
				{
					method: "POST",
					headers: createAuthHeader(userId),
					body: JSON.stringify({}),
	...
	describe("DELETE /api/trip/[tripSlug]/publish", () => {
		it("should unpublish public trip for authenticated owner", async () => {
			const request = new NextRequest(
				`http://localhost/api/trip/${publicTrip.slug}/publish`,
				{
					method: "DELETE",
	...
	describe("POST /api/trip/[tripSlug]/replica", () => {
		it("should create replica from public template", async () => {
			...
			const request = new NextRequest(
				`http://localhost/api/trip/${templateTrip.slug}/replica`,
				{
					method: "POST",
					headers: createAuthHeader(userId),
					body: JSON.stringify({
		...
		it("should deny creating replica from private template for non-owner", async () => {
			...
			await db
				.collection(COLLECTIONS.TRIPS)
				.doc(privateTemplate.id)
				.set(privateTemplate);

			const request = new NextRequest(
				`http://localhost/api/trip/${privateTemplate.slug}/replica`,
				{
					method: "POST",
					headers: createAuthHeader(userId), // 別のユーザーで認証
					body: JSON.stringify({
			...
			const response = await handleReplicateTrip(
				request,
				privateTemplate.slug!,
			);
		...
		it("should return 400 if trip is not a template", async () => {
			const request = new NextRequest(
				`http://localhost/api/trip/${publicTrip.slug}/replica`,
				{
					method: "POST",
					headers: createAuthHeader(userId),
					body: JSON.stringify({
		...
		it("should return 404 for non-existent trip", async () => {
			const request = new NextRequest(
				"http://localhost/api/trip/non-existent-trip/replica",
				{
					method: "POST",
					headers: createAuthHeader(userId),
					body: JSON.stringify({
