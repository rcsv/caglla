...
import {
	...
	UpdateTripCommentSchema,
} from "@/lib/schemas/trip-social";
		...
		if (error instanceof Error) {
			...
			if (error.message.includes("Comments available only for public trips")) {
				...
				return NextResponse.json(
					{ error: "Comments available only for public trips" },
					{ status: 403 },
				);
			}
		}
...
export async function POST(
	...
	try {
		...
		if (error instanceof Error) {
			...
			if (
				error.message.includes("Parent comment does not belong to this trip")
			) {
...
export async function PUT(
	...
	try {
		...
		if (error instanceof Error) {
			...
			if (
				error.message.includes("Only comment owner can update") ||
...
export async function DELETE(
	...
	try {
		...
		if (error instanceof Error) {
			...
			if (
				error.message.includes("Only comment owner can delete") ||
				error.message.includes("owner")
			) {
				// 403 Forbidden は標準的なエラーレスポンスなので、そのまま返す
				return NextResponse.json(
