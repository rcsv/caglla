...
export async function GET(
	...
	try {
			...
			if (authHeader && authHeader.startsWith("Bearer ")) {
				...
				if (idToken) {
					...
					if (decodedToken) {
						...
						const user = await adminUserOperations
							.getUserByAuthUid(userId)
							.catch(() => null);
						if (user) {
		...
		if (
			...
			logger.debug(
				"Template trip has no days, generating days based on day_count",
				{
					tripId,
			...
			for (const dayData of daysToCreate) {
				try {
					...
					logger.error("Failed to create day for template trip", error, {
		...
		const daysWithItineraries = await Promise.all(
			days.map(async (day) => {
				const rawItineraries =
					await adminItineraryOperations.getItinerariesByDayId(day.id);
							...
							try {
								...
								for (const lang of fallbackLanguages) {
									...
									try {
										...
										logger.debug(
											`Failed to fetch PlacesCache with key: ${cacheKey}`,
											error,
										);
									}
								}
								...
								if (placesCache) {
									...
									logger.warn("PlacesCache not found for itinerary", {
										itineraryId: it.id,
										placeId: it.place_id,
										triedKeys: fallbackLanguages.map(
											(lang) => `${it.place_id}_${lang}`,
										),
									});
								}
							} catch (error) {
								logger.error(
									"Failed to resolve place_data for itinerary",
									error,
									{ itineraryId: it.id },
								);
							}
...
export const PUT = tripApi(async (request: NextRequest, ctx) => {
	...
	const normalizedDayCountInput =
		typeof bodyDayCount === "number"
			? bodyDayCount
			: typeof snakeDayCount === "number"
				? snakeDayCount
				: undefined;
	const normalizedDayCount =
		normalizedDayCountInput !== undefined &&
		Number.isFinite(normalizedDayCountInput) &&
		normalizedDayCountInput > 0
	...
	// 日程が変更された場合の処理
	const startDateChanged = !compareDates(
		toDateOrNull(originalStartDate) || undefined,
		newStartDate,
	);
	const endDateChanged = !compareDates(
		toDateOrNull(originalEndDate) || undefined,
		newEndDate,
	);
		...
		try {
			...
			existingDays.forEach((day) => {
				if (day.date) {
					...
					if (
						dayDateNormalized < startNormalized ||
						dayDateNormalized > endNormalized
					) {
