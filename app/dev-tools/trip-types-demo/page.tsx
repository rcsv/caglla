...
export default function TripTypesDemoPage() {
  ...
  const mockSharedMembers: User[] = [
    ...
    },
    {
      ...
      auth_uid: 'auth-carol',
      profile_image_url: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Carol',
      created_at: new Date(),
      updated_at: new Date(),
    },
  ]
  ...
  const privateTrip: ExtendedTrip = {
    ...
    days: mockDays.map(day => ({
      ...day,
      date: new Date(futureDate.getTime() + (day.day_number - 1) * 24 * 60 * 60 * 1000),
    })),
    social_stats: {
      likes_count: 0,
      comments_count: 0,
  ...
  const sharedTrip: ExtendedTrip = {
    ...
    days: mockDays.map(day => ({
      ...
      itineraries: day.itineraries?.map(it => ({
        ...it,
        // 宿泊先情報を匿名化
        location: it.location?.includes('ホテル') ? '那覇市内のホテル' : it.location,
        description: it.description?.includes('ホテル') ? 'ホテルにチェックイン' : it.description,
      })),
    })),
    social_stats: {
      likes_count: 15,
  ...
  const templateTrip: ExtendedTrip = {
    ...
    slug: 'trip-template',
    destination: '沖縄県',
    description: '沖縄を効率的に楽しむための3泊4日のプランです。地域の楽しみ方や移動方法を紹介します。',
    // 日付は設定しない
    status: 'PLANNING',
    access_level: 'public',
    is_template: true,
  ...
  const currentTrip = activeType === 'private' ? privateTrip : activeType === 'shared' ? sharedTrip : templateTrip
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4 max-w-6xl">
        {/* ヘッダー */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">旅行タイプデモ</h1>
          <p className="text-gray-600">3つの旅行タイプのユーザー体験を模擬できます</p>
        </div>
        ...
        {/* TripCard表示 */}
        <div className="mb-8">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">TripCard表示</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <Icon icon="mdi:information" className="h-6 w-6 text-indigo-600" />
              基本情報
            </h2>
            <div className="space-y-4">
                ...
                <p className="text-gray-900 mt-1 flex items-center gap-2">
                  ...
                  {currentTrip.is_template && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded text-xs">
                      <Icon icon="mdi:file-document-outline" className="h-3 w-3" />
                      プラン
                    </span>
                  )}
                </p>
              ...
              <div>
                <label className="text-sm font-medium text-gray-700">アクセスレベル</label>
                <p className="text-gray-900 mt-1">
                  ...
                  >
                    <Icon
                      icon={currentTrip.access_level === 'private' ? 'mdi:lock' : 'mdi:earth'}
                      className="h-3 w-3"
                    />
                    {currentTrip.access_level === 'private' ? 'プライベート' : 'パブリック'}
                  </span>
                </p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700">説明</label>
                <p className="text-gray-600 mt-1 text-sm">{currentTrip.description}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <Icon icon="mdi:calendar-clock" className="h-6 w-6 text-indigo-600" />
              日程情報
            </h2>
            <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {currentTrip.is_shared && (
                      <span className="text-xs text-amber-600 bg-amber-50 px-2 py-0.5 rounded">
                        <Icon icon="mdi:shield-lock" className="h-3 w-3 inline mr-1" />
                        プライバシー保護
                      </span>
                    )}
                    {currentTrip.is_shared && day.itineraries?.some(it => it.location?.includes('ホテル')) && (
                      <span className="text-xs text-gray-500">（宿泊先情報は匿名化）</span>
                    )}
                  </div>
                  <div className="space-y-2">
                    {day.itineraries?.map((it) => (
                      <div key={it.id} className="text-sm text-gray-600 pl-4 border-l-2 border-indigo-200">
        ...
        {activeType === 'private' && (
          <div className="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <Icon icon="mdi:account-group" className="h-6 w-6 text-indigo-600" />
              招待メンバー
            </h2>
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-blue-800">
                  <p className="font-medium mb-1">この旅行は以下のメンバーと共有されています</p>
                  <p className="text-xs">会議の参加者リストのように、誰がこの旅行にアクセスできるかが表示されます。</p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3 p-3 bg-indigo-50 border border-indigo-200 rounded-lg">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-gray-900">{mockUser.name}</span>
                ...
                <Icon icon="mdi:check-circle" className="h-5 w-5 text-indigo-600" />
              </div>
        ...
        {activeType === 'template' && (
          <div className="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <Icon icon="mdi:account-circle" className="h-6 w-6 text-indigo-600" />
              クリエイター
            </h2>
            <div className="mb-4 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
              <div className="flex items-start gap-2">
                <Icon icon="mdi:information" className="h-5 w-5 text-emerald-600 flex-shrink-0 mt-0.5" />
        ...
        {activeType === 'shared' && (
          <div className="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <Icon icon="mdi:image-multiple" className="h-6 w-6 text-indigo-600" />
              旅行の写真
            </h2>
            <div className="mb-4 p-3 bg-purple-50 border border-purple-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-purple-800">
                  <p className="font-medium mb-1">アップロードした写真がitinerariesと連動して表示されます</p>
                  <p className="text-xs">InstagramやFacebookの写真投稿のように、ビジュアル重視のタイムライン形式で表示されます。</p>
                </div>
              </div>
            </div>
                ...
                <div key={day.id} className="border border-gray-200 rounded-lg overflow-hidden">
                  ...
                  <div className="bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold">
                        {currentTrip.is_shared ? `${day.day_number}日目` : day.date ? `${new Date(day.date).getMonth() + 1}月${new Date(day.date).getDate()}日（${day.day_number}日目）` : `${day.day_number}日目`}
                      </span>
                  ...
                  <div className="divide-y divide-gray-200">
                    {day.itineraries?.map((itinerary, itIndex) => {
                      ...
                      return (
                        <div key={itinerary.id} className="p-4 hover:bg-gray-50 transition-colors">
                              ...
                              <div className="relative aspect-video rounded-lg overflow-hidden bg-gray-200 group cursor-pointer">
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                                  <div className="absolute bottom-2 left-2 right-2 text-white text-xs">
                                    <Icon icon="mdi:map-marker" className="h-4 w-4 inline mr-1" />
                                ...
                                {itIndex === 0 && (
                                  <div className="absolute top-2 right-2 bg-black/60 text-white text-xs px-2 py-1 rounded flex items-center gap-1">
                                    <Icon icon="mdi:image-multiple" className="h-3 w-3" />
                                    <span>3</span>
                                  </div>
                                )}
                              </div>
        ...
        <div className="mt-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          ...
          <ul className="text-sm text-yellow-700 space-y-1">
            <li>• これはユーザー体験を模擬するためのハリボテ（モック）ページです</li>
