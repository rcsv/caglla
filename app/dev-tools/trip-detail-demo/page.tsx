...
const isLodgingVenue = (itinerary: Itinerary) => {
	const targetText =
		`${itinerary.title ?? ""} ${itinerary.description ?? ""} ${itinerary.location ?? ""}`.toLowerCase();
	return lodgingKeywords.some((keyword) =>
...
export default function TripDetailDemoPage() {
	...
	const [mapFocusMode, setMapFocusMode] = useState<"all" | "day" | "single">(
		"all",
	);
	...
	const typeTabs: {
	...
	}[] = [
		{
			...
			icon: "mdi:lock",
			description:
				"招待されたメンバーだけが、日付つきの詳細な日程を確認できる通常モード。",
		},
		{
			id: "shared",
			...
			description:
				"プライベート旅行を参考用にシェア。日付や宿泊先はプライバシー保護のためぼかして表示。",
		},
		{
			id: "template",
			...
			description:
				"日付なしで「何泊何日・Day1/Day2構成」だけを共有する旅行プラン（テンプレート）。",
		},
	];

	const activeTabMeta =
		typeTabs.find((t) => t.id === activeType) ?? typeTabs[0];
				...
				<div className="px-4 mt-4">
					{trip.demoType === "private" && (
						<div className="inline-flex items-center gap-2 rounded-full bg-indigo-50 px-3 py-1 text-xs text-indigo-700 border border-indigo-100">
							...
							<span>
								プライベート旅行：招待されたメンバーだけが、この詳細画面を開くことができます。
							</span>
						</div>
					)}
					{trip.demoType === "shared" && (
						<div className="flex flex-wrap gap-2">
							<div className="inline-flex items-center gap-2 rounded-full bg-amber-50 px-3 py-1 text-xs text-amber-800 border border-amber-100">
								...
								<span>
									プライバシー保護：日程や宿泊先は、参考用の粒度でのみ表示されています。
								</span>
							</div>
					...
					{trip.demoType === "template" && (
						<div className="inline-flex items-center gap-2 rounded-full bg-purple-50 px-3 py-1 text-xs text-purple-800 border border-purple-100">
							...
							<span>
								旅行プラン（テンプレート）：日付は未設定で、Day1 / Day2
								構成だけを共有します。
							</span>
						</div>
					)}
							...
							<div className="border-t border-gray-200 pt-4 mt-4">
								...
								<div className="space-y-4 mt-6">
									{[
										{
											name: "Alice Traveler",
											text: "素敵なプランですね！参考にさせていただきます。",
											time: "2時間前",
										},
										{
											name: "Bob Explorer",
											text: "京都のカフェ巡り、私も行ってみたいです！",
											time: "5時間前",
										},
										{
											name: "Carol Adventurer",
											text: "写真が美しいですね。実際に行った感想も聞きたいです。",
											time: "1日前",
										},
									].map((comment, idx) => (
