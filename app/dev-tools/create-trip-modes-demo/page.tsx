...
export default function CreateTripModesDemoPage() {
	const [selectedPublicTrip, setSelectedPublicTrip] = useState<Trip | null>(
		null,
	);
	...
	const mockPublicTrips: Trip[] = [
		...
		},
		{
			...
			creator: {
				...
				slug: "okinawa-expert",
				profile_image_url:
					"https://api.dicebear.com/7.x/avataaars/svg?seed=Okinawa",
				created_at: new Date(),
				updated_at: new Date(),
			} as User,
			social_stats: {
					...
					<div className="flex items-center justify-between mb-4">
						<div>
							<h1 className="text-3xl font-bold text-gray-900 mb-2">
								Create Trip from Public Trip
							</h1>
							<p className="text-gray-600">
								Public Tripをベースに新しいTripを作成する体験を模擬できます
							</p>
				...
				<div className="mt-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
					<div className="flex items-start gap-2">
						...
						<div className="text-sm text-yellow-800">
							...
							<p>
								このページは動作確認用のハリボテです。実際のTrip作成機能は未実装です。
							</p>
							<p className="mt-2">
								<strong>将来的な改善案：</strong> Public Tripカードに「Clone
								Trip
								Plan」ボタンを追加し、ワンクリックで複製できる体験を提供予定。
							</p>
						</div>
					</div>
				</div>
