		...
		async (triggeredByUser = false) => {
			...
			try {
				...
				if (!previewWindow) {
					...
					throw new Error(
						"ブラウザによってポップアップがブロックされました。許可後にもう一度お試しください。",
					);
				}

				previewWindow.document.write(
					'<p style="font-family:sans-serif;padding:16px;">Loading preview…</p>',
				);
				...
				if (!response.ok) {
					...
					try {
						...
						throw new Error(
							errorData.error || "プレビューの読み込みに失敗しました",
						);
					} catch {
		...
		return (
			<div className="min-h-screen bg-gray-50 flex items-center justify-center">
				<div className="text-center">
					...
					<p className="text-gray-600">トリップ: {tripSlug}</p>
				</div>
			</div>
		);
	}
		...
		<div className="min-h-screen bg-gray-50 flex items-center justify-center">
			<div className="text-center max-w-md mx-auto p-6">
				...
				<div className="bg-white p-4 rounded-lg border mb-6">
					...
					<code className="text-sm text-gray-600 break-all">{previewUrl}</code>
				</div>
				...
				<div className="mt-6 text-sm text-gray-500">
					...
					{popupBlocked && (
						<p className="mt-4 text-red-500">
							⚠️
							ブラウザがポップアップをブロックしています。許可設定を変更したあと、もう一度ボタンを押してください。
						</p>
					)}
				</div>
			</div>
