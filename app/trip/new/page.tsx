...
export default function NewTripPage() {
	...
	const { can, hasFeature, getRemaining, getLimitExceededMessage } =
		useSubscription();
		...
		try {
			...
			if (response.ok) {
				...
				if (trip.creator?.slug && trip.slug) {
					...
					logger.error("Trip created but slugs are missing", {
						tripId: trip.id,
					});
				...
				if (formData.imageUrl) {
					try {
						...
						logger.error(
							"Failed to delete image after creation failure:",
							error,
						);
					}
				}
										...
										<div>
											...
											>
												<option value="private">
													{t("tripNew.accessLevel.private")}
												</option>
												<option value="public">
													{t("tripNew.accessLevel.public")}
												</option>
											</select>
										</div>
									</div>
