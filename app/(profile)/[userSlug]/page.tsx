		...
		try {
			...
			if (res.ok) {
				...
				setEditForm({
					...
					unit_system:
						userPreferences.unit_system ||
						getDefaultUnitSystem(userPreferences.home_country_code),
				});

				// 初回セットアップの判定（bio、home_country_code、genderがすべて空の場合）
				const needsSetup =
					!data.user.bio &&
					!data.user.preferences?.home_country_code &&
					(!data.user.gender || data.user.gender === "prefer_not_to_say");
			...
			if (tripsRes.ok) {
				...
				const publicTripsList = trips.filter(
					(t) => t.access_level === "public",
				);
				...
				if (isOwnProfileCheck) {
					...
					if (isDev) {
						console.log(
							"🔒 Private trips filtered:",
							privateTripsList.length,
							privateTripsList.map((t) => ({
								id: t.id,
								title: t.title,
								access_level: t.access_level,
							})),
				...
				if (shouldShowStats) {
					const statsRes = await makeAuthenticatedRequest(
						"/api/trips?groupByCountry=true",
					);
					if (statsRes.ok) {
		...
		<div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
			...
			<main className="container mx-auto px-6 py-12">
				<div className="page-rail space-y-20">
					...
					{/* User Profile */}
					<Section
						title={isFirstTimeSetup ? t("profile.setup") : t("profile.title")}
					>
						<SolidCard className="p-8 md:p-10">
										...
										{isEditing || isFirstTimeSetup ? (
												...
												{editForm.home_country_code && (
													<div className="text-sm text-gray-700 flex items-center gap-2">
														<span className="text-sm">
															{t("profile.estimatedCountry")}
														</span>
														<span className="text-lg">
															{getCountryFlag(editForm.home_country_code)}
														</span>
														<span>
															{getCountryNameJa(editForm.home_country_code)}
														</span>
											...
											<div>
												...
												<div className="space-y-2 text-gray-600">
													...
													{profileUser.gender &&
														profileUser.gender !== "prefer_not_to_say" && (
															<p className="flex items-center gap-2">
														...
														<span className="text-gray-600">
															{profileUser.preferences?.language
																? LANGUAGE_NAMES[
																		profileUser.preferences
																			.language as SupportedLanguage
																	]?.native || profileUser.preferences.language
																: t("profile.language.auto")}
														</span>
													</p>
												</div>
							...
							{publicTrips.length === 0 ? (
								<SolidCard className="p-12 text-center">
									...
									<h4 className="text-xl font-semibold text-gray-900 mb-2">
										{t("profile.publicTrips.empty")}
									</h4>
									<p className="text-gray-600">
										{t("profile.publicTrips.empty.description")}
									</p>
								</SolidCard>
							) : (
													...
													{trip.destination && (
														<p className="text-gray-500 text-sm mb-3 flex items-center gap-1">
															...
															{trip.destination_place?.address_components && (
																<span>
																	{getCountryFlag(
																		trip.destination_place.address_components.find(
						...
						<Section title={t("profile.privateTrips.title")}>
							{privateTrips.length === 0 ? (
								<SolidCard className="p-12 text-center">
									...
									<h4 className="text-xl font-semibold text-gray-900 mb-2">
										{t("profile.privateTrips.empty")}
									</h4>
									<p className="text-gray-600">
										{t("profile.privateTrips.empty.description")}
									</p>
									{/* デバッグ情報 */}
									{typeof window !== "undefined" &&
										typeof process !== "undefined" &&
										process.env?.NODE_ENV === "development" && (
											<div className="mt-4 p-4 bg-gray-100 rounded text-left text-xs">
												...
												<p>
													Debug: profileUser.google_id={profileUser?.google_id}
												</p>
											</div>
										)}
								</SolidCard>
									...
									<div className="grid grid-cols-2 gap-6">
										<div className="bg-emerald-50 rounded-lg p-6 text-center">
											...
											<div className="text-gray-700 font-medium">
												{t("profile.stats.totalTrips")}
											</div>
										</div>
										<div className="bg-blue-50 rounded-lg p-6 text-center">
											<div className="text-4xl font-bold text-blue-600 mb-2">
												{tripStats.totalCountries}
											</div>
											<div className="text-gray-700 font-medium">
												{t("profile.stats.totalCountries")}
											</div>
										</div>
									</div>
									...
									{tripStats.countryGroups.length > 0 && (
										<div>
											<h4 className="text-lg font-semibold text-gray-900 mb-4">
												{t("profile.stats.countries.title")}
											</h4>
											<div className="space-y-3">
												{tripStats.countryGroups
													.slice(0, 5)
													.map((group, index) => (
														<div
											...
											{tripStats.countryGroups.length > 5 && (
												<p className="text-sm text-gray-500 mt-4 text-center">
													{t("profile.stats.countries.more").replace(
														"{count}",
														String(tripStats.countryGroups.length - 5),
													)}
												</p>
											)}
										</div>
