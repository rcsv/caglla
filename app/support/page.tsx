    ...
    <StaticPageLayout>
          ...
          <div className="lg:col-span-3 flex items-end">
            <div className="relative z-10 bg-white/85 backdrop-blur-sm p-6 border border-gray-200">
              <p className="text-lg md:text-xl text-gray-800 leading-relaxed">
                Caglla のサポートセンター。検索・カテゴリ・FAQ・お問い合わせで、素早く自己解決できます。
              </p>
            </div>
          </div>
        </div>
                    ...
                    <ul className="mt-2 space-y-1 text-sm text-indigo-600">
                      {c.links.map((l) => (
                        <li key={l.href} className="truncate">
                          ...
                          <span className="underline decoration-dotted">{l.label}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
        ...
        <SolidCard className="p-6 md:p-8">
          <div className="space-y-2">
            {filteredFaqs.map((item) => (
              <details key={item.id} id={item.id} className="group bg-white border border-gray-200 p-4 open:shadow-sm">
      ...
      <Section title="Quick Links">
          ...
          <SolidCard className="p-6 hover:shadow-sm transition">
            <a href="/faq" className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600">よくある質問をまとめて確認</div>
              </div>
            </a>
          </SolidCard>
          <SolidCard className="p-6 hover:shadow-sm transition">
        ...
        <div className="mt-6 text-xs text-gray-500">
          参考: モダンなサポートページの考え方（カテゴリー化・検索・FAQ・コンタクト）—
      ...
      <section className="text-center">
        <div className="bg-emerald-600 p-12 text-white">
          ...
          <p className="text-xl mb-8 opacity-90">解決しない場合はサポートへお気軽にご連絡ください。</p>
