  ...
  const fetchFollowingFeed = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        logger.error('Failed to fetch following feed', { status: res.status, error: errorText })
        throw new Error(`Failed to fetch following feed: ${res.status}`)
      }

      const json = await res.json()

      logger.debug('Following feed fetched successfully', { count: json.trips?.length ?? 0 })
      setTrips(json.trips ?? [])
