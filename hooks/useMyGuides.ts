...
export function useMyGuides(status: 'draft' | 'published' | 'all' = 'all'): UseMyGuidesResult {
  ...
  const fetchMyGuides = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        logger.error('Failed to fetch my guides', { status: res.status, error: errorText })
        throw new Error(`Failed to fetch my guides: ${res.status}`)
      }

      const json = await res.json()

      logger.debug('My guides fetched successfully', { count: json.trips?.length ?? 0 })
      setTrips(json.trips ?? [])
    } catch (e: any) {
