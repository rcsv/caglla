...
### Added
- **チェックリスト機能の大幅強化**
  - `longDescription`フィールドの追加（詳細説明文、右ペイン用）
  - Gemini API統合による自動生成機能
  - ユーザーメモ機能（`userMemo`フィールド）
  - 2カラムMaster-Detailレイアウト（左: アイテム一覧、右: 詳細パネル）
  - 折りたたみ可能な補助情報（ユーザーメモ、進捗状況、リンク、関連アイテム）
  - Read More方式の`longDescription`表示（3行表示+展開）
  - カテゴリ内の進捗状況表示
  - 関連アイテム表示（同じカテゴリの他のアイテム）
  - チェックリスト再生成時のユーザーデータ保持（`done`状態、`userMemo`、カスタムアイテム）

- **Gemini API統合**
  - `longDescription`の自動生成機能
  - 全ユーザー間で共有されるキャッシュシステム（`CHECKLIST_LONG_DESCRIPTION_CACHE`）
  - レート制限対応のバッチ処理（10件/分）
  - 並列処理による生成時間の短縮

- **UI/UX改善**
  - 優先度バッジの右寄せ配置
  - チェック済みアイテムのフェードアウト表示（取り消し線から変更）
  - カテゴリヘッダーへのアイコン追加
  - カテゴリ名から「関連」テキストの削除
  - 固定ヘッダー（カテゴリ、タイトル、優先度）の実装
  - スクロール可能コンテンツエリアの分離

### Changed
- **チェックリストレイアウト**
  - 3カラムレイアウトから2カラムMaster-Detailレイアウトに変更
  - 左ペイン: 40%、右ペイン: 60%の比率に調整
  - 地図非表示時の画面スペース活用

- **チェックリスト再生成ロジック**
  - 既存アイテムの`done`状態と`userMemo`を保持
  - カスタムアイテムの保持
  - `longDescription`の優先順位（新規生成 > 既存）

- **Gemini APIプロンプト**
  - 簡潔で実用的な文章生成（300-500文字）
  - 「ポイント」セクションの追加
  - 実用性重視のトーンに調整

### Fixed
- チェックリスト再生成時の認証エラーを修正
- Firestore保存時の`undefined`フィールドエラーを修正
- マップ表示のバグ修正（チェックリスト表示時の非表示化）
- レート制限エラー（429）の発生を抑制
