...
- [x] Itinerary Cardで表示される画像が、places_cacheの高解像度画像を使用している ✅ 完了
- [x] 画像解像度が視覚的に改善されている ✅ 完了（最大1600pxまで対応）
- [x] キャッシュがない場合のフォールバックが機能している ✅ 完了
- [ ] パフォーマンスに大きな影響がない（画像読み込み時間など） - 要確認
- [ ] APIコストが過度に増加していない - 要確認

**更新**: 2025-01-XX - places_cacheの解像度情報（width/height）を活用して、最高解像度（最大1600px）の画像を表示するように修正。固定800pxから動的解像度選択に変更。
