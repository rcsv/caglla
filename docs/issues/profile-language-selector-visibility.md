...
- [x] 通常の表示モードで現在選択されている言語が表示される
- [x] 編集モードに入らずに現在の言語設定を確認できる
- [x] 言語情報が適切な場所に表示される（UXが良い）
- [x] 言語が未設定の場合は「自動（ブラウザ設定）」が表示される
- [x] 国旗アイコンや言語名で視覚的に分かりやすく表示される
- [x] 既存の編集モードでの言語選択機能も維持される（後方互換性）
- [x] ビルドエラーがない
- [x] ブラウザで動作確認済み（複数言語設定での表示テスト）

## 📝 実装内容

プロフィールページの通常表示モード（`!isEditing && !isFirstTimeSetup`）で、他のプロフィール情報（メール、居住地、性別など）と同じ形式で言語設定を表示するように実装しました。

### 実装詳細

1. **言語フラグマッピングの追加**
   - `HomeHeader.tsx`と同様の`languageFlags`マッピングを追加
   - サポートされている全言語に対応

2. **言語表示の追加**
   - プロフィール情報の表示セクション（性別表示の下）に言語情報を追加
   - 国旗アイコンと言語名を表示
   - 言語が未設定（空文字列または`undefined`）の場合は「自動（ブラウザ設定）」を表示

3. **視覚的な一貫性**
   - 他のプロフィール情報と同じスタイルで表示
   - アイコン（国旗）とテキスト（言語名）の組み合わせ

### 実装ファイル

- `app/[userSlug]/page.tsx`: 言語表示UIの追加（464-476行目）
