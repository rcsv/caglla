...
**選定したロゴデザイン**: 飛行機 + 地図ピン（案1）

1. **LandingHeader.tsx**
   - 旧「家」アイコンを削除
   - `tabler:plane`と`tabler:map-pin`を組み合わせたロゴに変更
   - 飛行機を左上、地図ピンを右下に配置

2. **LandingFooter.tsx**
   - 同様に飛行機+地図ピンの組み合わせに変更
   - デザインを統一
...
- **アイコン**: Iconifyの`tabler:plane`と`tabler:map-pin`を使用
- **レイアウト**: 飛行機（左上）と地図ピン（右下）を重ねて配置
- **色**: エメラルドグリーンのグラデーション背景を維持
- **サイズ**: 既存の`w-8 h-8`コンテナを維持

### 対応済みの確認事項

- [x] 旅行を感じさせるロゴアイコンが選定されている（飛行機+地図ピン）
- [x] 非常口との混同がない（明確な旅行関連アイコン）
- [x] アイコンが潰れずに適切に表示される（absolute配置で重ね表示）
- [x] エメラルドグリーンのブランドカラーを維持
- [x] すべてのロゴ表示箇所に適用されている（LandingHeader, LandingFooter）
- [ ] ファビコンが更新されている（別途対応が必要）
- [x] 小さいサイズでも認識しやすい（アイコンサイズを調整）

---

## ✅ 完了条件

- [x] 旅行を感じさせるロゴアイコンが選定されている
- [x] 非常口との混同がない
- [x] アイコンが潰れずに適切に表示される
- [x] エメラルドグリーンのブランドカラーを維持（または新色を選定）
- [x] すべてのロゴ表示箇所に適用されている
- [ ] ファビコンが更新されている（将来的な対応）
- [x] 小さいサイズでも認識しやすい
