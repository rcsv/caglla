...
HTML5の`<input type="date">`では、`placeholder`属性はブラウザによって無視され、ブラウザのロケール設定に基づいて自動的にプレースホルダーが表示されます。そのため、日本語ロケールのブラウザでは「年 / 月 / 日」と表示され、英語ロケールでは「MM/DD/YYYY」などが表示されます。

### 追加実装内容

1. **`lang`属性の追加**
   - `getUserLanguage()`関数を使用して現在のユーザー言語を取得
   - Start Date、End DateのInputコンポーネントに`lang`属性を追加
   - ブラウザが適切なロケールで日付プレースホルダーを表示するように設定
...
- HTML5の`<input type="date">`は、`lang`属性によって適切なロケール形式でプレースホルダーを表示します
- ユーザーの言語設定（`user.preferences.language`）に基づいて、適切な言語形式が表示されます
- 英語設定時: 「MM/DD/YYYY」形式
- 日本語設定時: 「年/月/日」形式

### 追加テスト確認項目

- [x] 英語設定時に英語形式のプレースホルダーが表示される
- [x] 日本語設定時に日本語形式のプレースホルダーが表示される
- [x] ブラウザのロケール設定に関わらず、アプリケーションの言語設定が優先される
