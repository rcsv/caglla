...
describe('User Follows Social Operations', () => {
  ...
  describe('followUser', () => {
    ...
    it('should throw error when trying to follow already followed user', async () => {
      ...
      // 実装により、エラーを投げる
      await expect(followUser(userId, otherUserId, db)).rejects.toThrow('Already following')
    })
  })
  ...
  describe('unfollowUser', () => {
    ...
    it('should throw error when trying to unfollow non-existing follow relationship', async () => {
      // 既にフォロー関係が存在しない場合
      // 実装により、エラーを投げる
      await expect(unfollowUser(userId, thirdUserId, db)).rejects.toThrow('Not following this user')
    })
  })
