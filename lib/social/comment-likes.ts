...
export async function toggleCommentLike(
  ...
  // 3. いいねドキュメントの参照
  const commentRef = firestore.collection(COLLECTIONS.TRIP_COMMENTS).doc(resolvedCommentId)
  const likeRef = firestore.collection(COLLECTIONS.COMMENT_LIKES).doc(`${userId}_${resolvedCommentId}`)
