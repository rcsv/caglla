...
export function validateClientEnvironment(
	...
	if (missingVars.length > 0) {
		...
		if (isDevelopment()) {
			...
			if (!options.suppressWarnings && !global.__envWarningShown) {
				logger.debug(
					"🔧 Development mode: Environment variables validation skipped",
				);
	...
	return {
		NEXT_PUBLIC_FIREBASE_API_KEY:
			process.env.NEXT_PUBLIC_FIREBASE_API_KEY || "dev-fallback",
		NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN:
		...
		NEXT_PUBLIC_GOOGLE_PLACES_API_KEY:
			process.env.NEXT_PUBLIC_GOOGLE_PLACES_API_KEY || "dev-google-places-key",
		NEXT_PUBLIC_GOOGLE_MAPS_API_KEY:
			process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || "dev-google-maps-key",
		NEXT_PUBLIC_UNSPLASH_ACCESS_KEY:
			process.env.NEXT_PUBLIC_UNSPLASH_ACCESS_KEY || "dev-unsplash-key",
	};
}
