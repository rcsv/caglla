...
describe('Social Type Definitions', () => {
  ...
  describe('TripComment Interface', () => {
    ...
    it('should match actual usage pattern from trip-comments.ts', () => {
      ...
      const actualUsage: Omit<TripComment, 'id'> & { user_avatar?: string; parent_comment_id?: string } = {
        trip_id: tripId,
        user_id: userId,
        user_name: userName,
  ...
  describe('Type Guards', () => {
    ...
    describe('isTripComment', () => {
      ...
      it('should return false for invalid TripComment', () => {
        ...
        expect(isTripComment({ id: 'test', trip_id: 'test', user_id: 'test' })).toBe(false)
        // user_nameが欠けている
        expect(isTripComment({
          id: 'test',
          trip_id: 'test',
          user_id: 'test',
          content: 'test',
