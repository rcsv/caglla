...
export class FailureLogger<T extends BaseFailureLog> {
	...
	save(log: Omit<T, "id">): void {
		...
		try {
			...
			if (existingLogs.length >= this.config.batchSize) {
				logger.warn(
					`Failure logs reached batch size (${this.config.batchSize}) for key: ${this.config.storageKey}`,
				);
			}
		} catch (error) {
	...
	markAsProcessed(logIds: string[]): void {
		...
		const updatedLogs = logs.map((log) =>
			logIds.includes(log.id) ? { ...log, status: "processed" as const } : log,
		);
	...
	markAsIgnored(logIds: string[]): void {
		...
		const logs = this.getLogs();
		const updatedLogs = logs.map((log) =>
			logIds.includes(log.id) ? { ...log, status: "ignored" as const } : log,
		);
