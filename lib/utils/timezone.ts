...
// タイムゾーン失敗ログ管理
const timezoneFailureLogger = new FailureLogger<TimezoneFailureLog>(
	"timezone_failure_logs",
	50,
);
...
export const timezoneUtils = {
	...
	getTimezoneFromPlace: (placeData: PlaceData, userId?: string): string => {
		...
		const placesToCheck = [placeData.name, placeData.formatted_address]
			.filter(Boolean)
			.map((s) => s!.toLowerCase());
	...
	updateCityTimezoneMapping: (updates: TimezoneMappingUpdate[]): void => {
		updates.forEach((update) => {
			if (update.confidence === "high" || update.confidence === "medium") {
				logger.debug(
					`Timezone mapping suggestion: ${update.city_name} -> ${update.timezone}`,
				);
