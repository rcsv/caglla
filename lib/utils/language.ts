...
export function getUserLanguage(
	...
	if (isServer) {
		...
		if (override) {
			logLanguageOnce(
				"serverOverride",
				"Language from server cookie override:",
				override,
			);
			return override;
		}
		...
		if (override && isSupportedLanguage(override)) {
			logLanguageOnce(
				"clientOverride",
				"Language from override (cookie/localStorage):",
				override,
			);
			return override;
		}
	...
	if (user?.preferences?.language !== undefined) {
		...
		if (userLang === "") {
			logger.debug(
				"User selected auto (browser) language, checking browser settings",
			);
			...
			if (typeof window !== "undefined" && typeof navigator !== "undefined") {
				...
				if (isSupportedLanguage(browserLang)) {
					logLanguageOnce(
						"userPreferenceAutoBrowser",
						"Language from browser (user selected auto):",
						browserLang,
					);
					return browserLang;
				}
...
export function parseCacheKey(cacheKey: string): {
	...
	const language = isSupportedLanguage(languageStr)
		? languageStr
		: DEFAULT_LANGUAGE;
