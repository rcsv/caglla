...
import {
	...
	getCurrencyByCityName,
} from "@/lib/core/locations";
...
// 通貨失敗ログ管理
const currencyFailureLogger = new FailureLogger<CurrencyFailureLog>(
	"currency_failure_logs",
	50,
);
...
export const currencyUtils = {
	...
	getCurrencyFromPlace: (placeData: PlaceData, userId?: string): string => {
		...
		const countryCode = placeData.address_components?.find((component: any) =>
	...
	getCurrencyFromPlaceEnhanced: (
		...
		if (placeData?.address_components) {
			const countryCode = placeData.address_components.find((component) =>
				component.types.includes("country"),
			)?.short_name;
			...
			logger.debug("📍 Step 1: Checking venue address_components", {
				...
				allComponents: placeData.address_components.map((c) => ({
					types: c.types,
			...
			if (countryCode) {
				...
				if (currency) {
					logger.debug(
						`✅ Currency inferred from venue country: ${countryCode} -> ${currency}`,
					);
					return { currency, source: "venue", confidence: "high" };
				} else {
					logger.debug(
						`⚠️ Country code ${countryCode} found but no currency mapping exists`,
					);
				}
			} else {
				logger.debug(
					"⚠️ Step 1: No country component found in address_components",
				);
			}
		} else {
		...
		if (placeData) {
			...
			if (cityCurrency) {
				logger.debug(
					`Currency inferred from city name: ${addressLower} -> ${cityCurrency}`,
				);
			...
			if (placeName) {
				...
				if (nameCurrency) {
					logger.debug(
						`Currency inferred from place name: ${placeName} -> ${nameCurrency}`,
					);
					return {
		...
		if (trip?.destination_place?.address_components) {
			...
			if (countryInfo.countryCode && countryInfo.countryCode !== "unknown") {
				...
				if (currency) {
					logger.debug(
						`Currency inferred from trip destination_place: ${countryInfo.countryCode} -> ${currency}`,
					);
		...
		if (trip?.destination) {
			...
			if (destinationCurrency) {
				logger.debug(
					`Currency inferred from trip destination string: ${destinationLower} -> ${destinationCurrency}`,
				);
				return {
		...
		if (user?.preferences?.home_country_code) {
			const currency = getCurrencyByCountryCode(
				user.preferences.home_country_code,
			);
			if (currency) {
				logger.debug(
					`Currency inferred from user home_country_code: ${user.preferences.home_country_code} -> ${currency}`,
				);
		...
		// 6. デフォルト（JPY）
		logger.debug(
			"❌ Currency inference failed at all steps, using default: JPY",
			{
	...
	// 失敗ログ管理
	getCurrencyFailureLogs: (): CurrencyFailureLog[] =>
		currencyFailureLogger.getLogs(),
	...
	analyzeCurrencyFailureLogs: (): CurrencyMappingUpdate[] => {
		...
		logs.forEach((log) => {
			if (log.detected_city) {
				cityCounts[log.detected_city] =
					(cityCounts[log.detected_city] || 0) + 1;
			}
			if (log.detected_country) {
				countryCounts[log.detected_country] =
					(countryCounts[log.detected_country] || 0) + 1;
			}
		});
	...
	updateCityCurrencyMapping: (updates: CurrencyMappingUpdate[]): void => {
		...
		updates.forEach((update) => {
			if (update.confidence === "high" || update.confidence === "medium") {
				logger.debug(
					`Updated currency mapping: ${update.city_name} -> ${update.currency}`,
				);
			}
		});
	},
	...
	// バッチ処理の実行
	processCurrencyBatchUpdate: (): {
		updates: CurrencyMappingUpdate[];
