...
import {
	dummyPaymentService,
	SubscriptionPlan as DummySubscriptionPlan,
	Subscription,
	PaymentMethod,
...
export function SubscriptionProvider({
	...
	const checkSubscription = useCallback(async () => {
		try {
			...
			const paymentMethods =
				await dummyPaymentService.getPaymentMethods("demo_user");
			...
			if (storedSubscriptionId) {
				const subscription =
					await dummyPaymentService.getSubscription(storedSubscriptionId);
				if (subscription) {
	...
	const subscribeToPlan = async (
		...
		try {
			...
			try {
				...
				if (!response.ok) {
					logger.error(
						"Failed to update plan in Firestore:",
						await response.text(),
					);
				} else {
	...
	const cancelSubscription = async (): Promise<boolean> => {
		try {
			...
			await dummyPaymentService.cancelSubscription(
	...
	const can = (type: RestrictionType, currentValue: number = 1): boolean => {
		const planId =
			(subscriptionStatus.plan?.id as PlanId) || PlanId.SEASON_TRAVELER;
	...
	const getLimitExceededMessage = (
		...
		const planId =
			(subscriptionStatus.plan?.id as PlanId) || PlanId.SEASON_TRAVELER;
		return RestrictionProvider.getLimitExceededMessage(
			planId,
			type,
			currentValue,
		);
