 ...
 */
export async function resolveUserIdFromSlug(
	userSlug: UserSlug,
): Promise<UserId | null> {
	try {
 ...
 */
export async function resolveUserSlugFromId(
	userId: UserId,
 ...
 */
export async function resolveTripSlugFromId(
	tripId: TripId,
): Promise<TripSlug | null> {
	try {
		const db = getSafeFirestore();
...
// クライアントサイド用の関数は lib/auth/client-identity-helpers.ts に移動しました
// このファイルはサーバーサイド専用です（Firebase Admin SDK に依存）
 ...
 * @returns UserId または null
 */
export async function adminResolveUserIdFromSlug(
	userSlug: UserSlug,
): Promise<UserId | null> {
	try {
 ...
 * @returns UserSlug または null
 */
export async function adminResolveUserSlugFromId(
	userId: UserId,
): Promise<UserSlug | null> {
	try {
 ...
 * @returns TripSlug または null
 */
export async function adminResolveTripSlugFromId(
	tripId: TripId,
): Promise<TripSlug | null> {
	try {
