...
export async function isSameUserById(
  ...
  try {
    ...
    const userDoc = await adminDb.collection(COLLECTIONS.USERS).doc(userDocumentIdOrGoogleId).get()
    ...
    const user = await adminUserOperations.getUserByAuthUid(userDocumentIdOrGoogleId)
    if (user) {
    ...
    logger.error('Error checking user identity', { error, authUid, userDocumentIdOrGoogleId })
    return false
  }
}
