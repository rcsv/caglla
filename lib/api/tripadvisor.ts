...
class TripAdvisorAPI {
	...
	constructor() {
		...
		if (!this.apiKey) {
			...
			logger.debug(
				"✅ TripAdvisor API key configured (length:",
				this.apiKey.length,
				")",
			);
		}
	...
	private async makeRequest<T>(
		...
		try {
			...
			logger.debug(
				...
				url.replace(this.apiKey, "REDACTED"),
			);
			...
			if (!response.ok) {
				...
				if (response.status === 403) {
					logger.error(
						"TripAdvisor 403 Forbidden - API key may be invalid or expired",
					);
				}
