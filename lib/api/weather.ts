...
export class WeatherApiHelpers {
	...
	static async getWeatherForecast(
		...
		if (adjustedStartDate > adjustedEndDate) {
			throw new Error(
				"Weather forecast is only available for dates within 16 days from today",
			);
		}
		...
		const params = new URLSearchParams({
			...
			daily:
				"temperature_2m_max,temperature_2m_min,weathercode,precipitation_sum,windspeed_10m_max,winddirection_10m_dominant",
			timezone: "Asia/Tokyo",
		});
		...
		if (!response.ok) {
			...
			if (response.status === 400) {
				throw new Error(
					"Weather API request error: Invalid parameters or date range",
				);
	...
	static async getTripWeather(
		...
		try {
			...
			if (!coordinates) {
				logger.warn(
					`Could not find coordinates for destination: ${destination}`,
				);
				// Return a fallback summary indicating no weather data available
				return this.createFallbackWeatherSummary(
					destination,
					startDate,
					endDate,
				);
			...
			const summary = this.calculateWeatherSummary(
				forecast,
				startDate,
				endDate,
			);
	 ...
	 */
	private static async getCoordinatesForDestination(
		destination: string,
		...
		if (googleApiKey) {
			try {
				...
				logger.debug(
					...
					googleGeocodingUrl.replace(googleApiKey, "***"),
				);
				...
				if (response.ok) {
					...
					if (data.status === "OK" && data.results && data.results.length > 0) {
						...
						logger.warn(
							"Google Geocoding API returned no results:",
							data.status,
						);
					}
				} else {
			...
			logger.warn(
				"Google Places API key not found, falling back to Open-Meteo",
			);
		}
		...
		try {
			...
			if (!response.ok) {
				...
				if (response.status === 403) {
					throw new Error(
						"Open-Meteo Geocoding API access denied. Please check rate limits or try again later.",
					);
				}
			...
			if (data.results && data.results.length > 0) {
				const result = data.results[0];
				logger.debug(
					"Found coordinates via Open-Meteo:",
					result.latitude,
					result.longitude,
					"for:",
					result.name,
	...
	private static calculateWeatherSummary(
		...
		const temperatures = daily.temperature_2m_max.map(
		...
		const rainyDays = daily.weathercode.filter(
			(code) =>
				(code >= 51 && code <= 67) ||
				(code >= 80 && code <= 82) ||
				(code >= 95 && code <= 99),
		...
		const averageWindSpeed =
			daily.windspeed_10m_max.reduce((sum, wind) => sum + wind, 0) /
