...
import type { TripAdvisorLocation, TripAdvisorReview, TripAdvisorPhoto } from './tripadvisor'
import type { FoursquareLocation, FoursquareTip, FoursquarePhoto } from './foursquare'
...
class VenueAggregator {
   ...
   */
  async getAggregatedVenueData(googlePlaceData: PlaceData): Promise<AggregatedVenueData> {
  ...
  private calculateAggregatedRating(
    ...
    const sources: Array<{ source: 'google' | 'tripadvisor' | 'foursquare'; rating: number; reviewCount: number }> = []
    ...
    const weightedRating = sources.reduce((sum, s) => sum + s.rating * s.reviewCount, 0) / totalReviews
  ...
  ): Array<{ url: string; source: 'google' | 'tripadvisor' | 'foursquare'; width?: number; height?: number }> {
    const photos: Array<{ url: string; source: 'google' | 'tripadvisor' | 'foursquare'; width?: number; height?: number }> = []
  ...
  getAggregatedPriceLevel(aggregatedData: AggregatedVenueData): {
    ...
    if (aggregatedData.google?.price_level !== undefined && aggregatedData.google.price_level >= 0) {
      return {
    ...
    if (aggregatedData.tripAdvisor.details?.price_level) {
      ...
      const priceLevel = priceMap[aggregatedData.tripAdvisor.details.price_level]
      if (priceLevel) {
