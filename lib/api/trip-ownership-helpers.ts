...
export async function checkTripOwnership(
	...
	if (trip.user_id === authUid) {
		logger.debug("Trip ownership: direct match", {
			tripUserId: trip.user_id,
			authUid,
		});
		return true;
	...
	try {
		...
		logger.error("Error checking trip ownership", {
			error,
			tripId: trip.id,
			authUid,
		});
	...
	try {
		...
		if (userDoc) {
			...
			logger.warn("User not found by trip.user_id (fallback)", {
				tripUserId: trip.user_id,
			});
		}
	} catch (error) {
		logger.error("Error checking trip ownership (fallback)", {
			error,
			tripId: trip.id,
			authUid,
		});
