...
export class UnsplashApiClient {
	...
	async getRandomPhoto(
		options: UnsplashRandomOptions = {},
	): Promise<UnsplashRandomResponse> {
		const params = new URLSearchParams({
		...
		return this.makeRequest<UnsplashRandomResponse>(
			`/photos/random${queryString ? `?${queryString}` : ""}`,
		);
	}
...
export function getUnsplashClient(): UnsplashApiClient {
	if (!unsplashClient) {
		...
		if (!accessKey) {
			throw new Error(
				"Unsplash access key is not configured. Please set NEXT_PUBLIC_UNSPLASH_ACCESS_KEY or UNSPLASH_ACCESS_KEY in your environment variables.",
			);
		}
...
export const unsplashApiHelpers = {
	...
	async getTravelPhotoOptions(
		...
		Array<{ url: string; description?: string; photographer: string }>
	> {
		try {
			const client = getUnsplashClient();
	 ...
	 * 画像のクレジット情報を取得
	 */
	async getPhotoCredit(
		photoUrl: string,
	): Promise<{ photographer: string; unsplashUrl: string } | null> {
		try {
			// 画像URLから写真IDを抽出（簡易実装）
			// 実際の実装では、より詳細な情報が必要
			return {
