 ...
 * ```
 */
export function withDayOwnership(options?: { source?: 'params' | 'body' | 'query' }): Middleware {
  ...
  return async (request: NextRequest, ctx: MiddlewareContext): Promise<MiddlewareContext | NextResponse> => {
    try {
      ...
      if (source === 'params') {
        if (!ctx.params) {
          return badRequest('Route parameters are required for day ownership check')
        }
        ...
        return badRequest('Body source not yet supported. Use params or query instead.')
      }
      
      if (!dayId || typeof dayId !== 'string') {
        return badRequest(`${source === 'params' ? 'dayId' : 'day_id'} parameter is required`)
      }
