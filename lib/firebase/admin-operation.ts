	...
	async createOrUpdateUser(
		...
		if (existingUser) {
			...
			if (userData.preferences) {
				...
				const cleanedPreferences = Object.entries(userData.preferences).reduce(
					(acc, [key, value]) => {
						if (value !== undefined) {
							acc[key] = value;
						}
			...
			if (
				...
				userData.profile_image_url !== existingUser.profile_image_url
			) {
			...
			if (
				...
				userData.gender !== existingUser.gender
			) {
			...
			const userDataWithAuthUid = {
				...
				google_id:
					userData.google_id || (userData.auth_uid ? undefined : undefined),
			};
...
export const adminTripOperations = {
	...
	async resolveTripByIdOrSlug(
		...
		if (byId.exists) {
			return {
				id: byId.id,
				trip: adminFirestoreHelpers.docToObject<Trip>(byId),
			};
		}
		...
		return {
			id: docSnap.id,
			trip: adminFirestoreHelpers.docToObject<Trip>(docSnap),
		};
	},
	async createTrip(
	...
	async getTripsByUserId(userId: string): Promise<Trip[]> {
		...
		const byDocumentId = await adminDb
			.collection(COLLECTIONS.TRIPS)
			.where("user_id", "==", userId)
			.get();

		if (!byDocumentId.empty) {
			const trips = byDocumentId.docs.map((doc: any) =>
				adminFirestoreHelpers.docToObject<Trip>(doc),
			);
		...
		if (user && user.id !== userId) {
			...
			const byUserDocumentId = await adminDb
				.collection(COLLECTIONS.TRIPS)
				.where("user_id", "==", user.id)
				.get();

			if (!byUserDocumentId.empty) {
				const trips = byUserDocumentId.docs.map((doc: any) =>
					adminFirestoreHelpers.docToObject<Trip>(doc),
				);
				return trips.sort((a: Trip, b: Trip) => {
					const aDate = toDateOrNull(a.created_at);
	...
	async deleteTrip(tripId: string): Promise<void> {
		...
		try {
			...
			if (tripDoc.exists) {
				...
				logger.info("Trip document found, checking for image:", {
					tripId,
			...
			logger.error("Failed to delete trip image before trip deletion:", {
				error,
				tripId,
			});
	...
	async deleteTripImage(imageUrl: string, tripId: string): Promise<void> {
		try {
			...
			if (!imageUrl || typeof imageUrl !== "string") {
				logger.warn("Invalid imageUrl provided to deleteTripImage:", {
					imageUrl,
					tripId,
				});
				return;
			...
			try {
				...
				if (!url.pathname.includes("/o/")) {
					logger.warn("URL does not appear to be a Firebase Storage URL:", {
						imageUrl,
						tripId,
					});
					return;
				...
				if (pathParts.length < 2) {
					logger.warn("Invalid Firebase Storage URL format:", {
						imageUrl,
						tripId,
					});
					return;
				...
				if (!pathWithParams) {
					logger.warn("No path found in Firebase Storage URL:", {
						imageUrl,
						tripId,
					});
					return;
				...
				logger.error("Error parsing image URL:", {
					error: urlError,
					imageUrl,
					tripId,
				});
			...
			logger.debug("Storage bucket obtained:", {
				bucketName: bucket.name,
				tripId,
			});

			const file = bucket.file(path);
			logger.debug("File reference created:", {
				path,
				fullPath: file.name,
				tripId,
			});
			...
			await file.delete();
			logger.info("Successfully deleted trip image from Storage:", {
				path,
				tripId,
			});
		} catch (error) {
	...
	async moveImageToTripPath(
		...
		try {
			...
			try {
				...
				throw new Error(
					`Failed to parse image URL: ${urlError instanceof Error ? urlError.message : String(urlError)}`,
				);
			}
			...
			const isAvatarPath =
				oldPath.includes("users/") && oldPath.includes("/avatar/");
			if (!isAvatarPath) {
				logger.warn("Image path is not an avatar path, skipping move:", {
					oldPath,
					tripId,
				});
			...
			if (!exists) {
				logger.warn("Source image does not exist, skipping move:", {
					oldPath,
					tripId,
				});
			...
			logger.info("Image move operation completed successfully:", {
				oldPath,
				newPath,
				newUrl,
				tripId,
	...
	async getPublicTrips(): Promise<Trip[]> {
		...
		const trips = querySnapshot.docs.map((doc: any) =>
			adminFirestoreHelpers.docToObject<Trip>(doc),
		);
...
export const adminDayOperations = {
	async createDay(
		dayData: Omit<Day, "id" | "created_at" | "updated_at">,
	): Promise<Day> {
		// undefined値をフィルタリング
	...
	async getDaysByTripId(tripId: string): Promise<Day[]> {
		...
		const days = querySnapshot.docs.map((doc: any) =>
			adminFirestoreHelpers.docToObject<Day>(doc),
		);
	...
	async updateDaysForTripAtomic(
		...
		await adminDb.runTransaction(async (transaction: any) => {
			// 既存のdayドキュメントを取得
			const existingDaysQuery = adminDb
				.collection(COLLECTIONS.DAYS)
				.where("trip_id", "==", tripId);
...
export const adminItineraryOperations = {
	async createItinerary(
		itineraryData: Omit<Itinerary, "id" | "created_at" | "updated_at">,
	): Promise<Itinerary> {
	...
	async getItinerariesByDayId(dayId: string): Promise<Itinerary[]> {
		...
		const itineraries = querySnapshot.docs.map((doc: any) =>
			adminFirestoreHelpers.docToObject<Itinerary>(doc),
		);

		// Sort by sort_number on the client side (ascending)
		return itineraries.sort(
			(a: Itinerary, b: Itinerary) => a.sort_number - b.sort_number,
		);
	},
...
export const adminTripUserOperations = {
	...
	async getUsersByTripId(tripId: string): Promise<TripUser[]> {
		...
		return querySnapshot.docs.map((doc: any) =>
			adminFirestoreHelpers.docToObject<TripUser>(doc),
		);
	},
	...
	async getTripsByUserId(userId: string): Promise<TripUser[]> {
		...
		const tripUsers = querySnapshot.docs.map((doc: any) =>
			adminFirestoreHelpers.docToObject<TripUser>(doc),
		);
