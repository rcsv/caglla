...
export async function getUserTripsWithBackwardCompatibility(
...
): Promise<{ trips: Trip[]; lastDoc: FirebaseFirestore.QueryDocumentSnapshot | null }> {
  ...
  const queryPromises = ownerIds.map(async (ownerId) => {
    let query: FirebaseFirestore.Query = db.collection(COLLECTIONS.TRIPS)
    ...
    if (additionalFilters?.accessLevel && typeof additionalFilters.accessLevel === 'string') {
  ...
  if (orderBy) {
    uniqueDocs.sort((a, b) => {
      ...
      const aDate = aValue instanceof Date ? aValue.getTime() : new Date(aValue as any).getTime()
      const bDate = bValue instanceof Date ? bValue.getTime() : new Date(bValue as any).getTime()
 ...
 */
export function encodeCursor(doc: FirebaseFirestore.QueryDocumentSnapshot): string {
  const data = doc.data()
 ...
 */
export function decodeCursor(encodedCursor: string): { timestamp: number; docId: string } | null {
  try {
