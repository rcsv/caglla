...
export class PlanLimitChecker {
	...
	static checkTravelCountLimit(
		...
		const remaining =
			limit === -1 ? -1 : Math.max(0, limit - currentTravelCount);
	...
	static checkAllLimits(
		...
		const publicTemplate = this.checkPublicTemplateLimit(
			plan,
			usage.publicTemplateCount ?? 0,
		);
	...
	static generateLimitExceededMessage(
		...
		if (!checks.travelCount.isAllowed) {
			exceededLimits.push(
				`旅行データ数が制限を超過しています (${checks.travelCount.currentUsage}/${checks.travelCount.limit}件)`,
			);
		}
		if (!checks.travelDays.isAllowed) {
			exceededLimits.push(
				`旅行日数が制限を超過しています (${checks.travelDays.currentUsage}/${checks.travelDays.limit}日)`,
			);
		}
		if (!checks.storage.isAllowed) {
			exceededLimits.push(
				`ストレージ容量が制限を超過しています (${checks.storage.currentUsage.toFixed(2)}/${checks.storage.limit}GB)`,
			);
		}
		if (!checks.photos.isAllowed) {
			exceededLimits.push(
				`写真アップロード数が制限を超過しています (${checks.photos.currentUsage}/${checks.photos.limit}枚)`,
			);
		}
		if (!checks.publicTemplate.isAllowed) {
			exceededLimits.push(
				"公開テンプレートの作成にはBackpacker以上のプランが必要です",
			);
		}
	...
	static generateUpgradeMessage(
		...
		if (
			checks.travelCount.limit !== -1 &&
			checks.travelCount.currentUsage / checks.travelCount.limit >= 0.8
		) {
			nearLimitItems.push("旅行データ数");
		}
		if (
			checks.travelDays.limit !== -1 &&
			checks.travelDays.currentUsage / checks.travelDays.limit >= 0.8
		) {
			nearLimitItems.push("旅行日数");
		}
		if (
			checks.storage.limit !== -1 &&
			checks.storage.currentUsage / checks.storage.limit >= 0.8
		) {
			nearLimitItems.push("ストレージ容量");
		}
		if (
			checks.photos.limit !== -1 &&
			checks.photos.currentUsage / checks.photos.limit >= 0.8
		) {
