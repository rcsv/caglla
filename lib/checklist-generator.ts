...
export class ChecklistGenerator {
  ...
  private getContinentCode(countryCode: string): string {
    const continentMap: { [key: string]: string } = {
      // Asia
      'JP': 'AS', 'CN': 'AS', 'KR': 'AS', 'TH': 'AS', 'VN': 'AS', 'IN': 'AS', 'ID': 'AS', 'MY': 'AS', 'SG': 'AS', 'PH': 'AS',
      // Europe
      'GB': 'EU', 'FR': 'EU', 'DE': 'EU', 'IT': 'EU', 'ES': 'EU', 'NL': 'EU', 'BE': 'EU', 'CH': 'EU', 'AT': 'EU', 'GR': 'EU',
      // North America
      'US': 'NA', 'CA': 'NA', 'MX': 'NA',
      // South America
      'BR': 'SA', 'AR': 'SA', 'CL': 'SA', 'PE': 'SA', 'CO': 'SA',
      // Africa
      'EG': 'AF', 'ZA': 'AF', 'KE': 'AF', 'NG': 'AF', 'MA': 'AF',
      // Oceania
      'AU': 'OC', 'NZ': 'OC', 'FJ': 'OC',
    }
    ...
    switch (condition.type) {
      ...
      case 'destination':
        ...
        if (condition.continents && context.destination.continentCode) {
          if (!condition.continents.includes(context.destination.continentCode)) {
            return false
          }
        }
        return true
