...
function createMockTripUser(tripId: string, userId: string, overrides?: Partial<TripUser>): TripUser {
  return {
    id: `${userId}_${tripId}`,
    trip_id: tripId,
    user_id: userId,
...
describe('getAccessibleTrips', () => {
  ...
  describe('所有Tripの取得', () => {
    it('should return owned trips only when includeShared=false', async () => {
      ...
      const result = await getAccessibleTrips(userId, { includeShared: false }, db)
  ...
  describe('共有Tripの取得', () => {
    it('should return both owned and shared trips when includeShared=true', async () => {
      ...
      await db.collection(COLLECTIONS.TRIP_USERS).doc(tripUser.id).set(tripUser)

      const result = await getAccessibleTrips(userId, { includeShared: true }, db)
  ...
  describe('ページネーション', () => {
    it('should respect limit parameter', async () => {
      ...
      const result = await getAccessibleTrips(userId, { limit: 5, includeShared: false }, db)
    ...
    it('should support pagination with cursor', async () => {
      ...
      // 最初のページを取得
      const firstResult = await getAccessibleTrips(userId, { limit: 2, includeShared: false }, db)
  ...
  describe('フィルタリング', () => {
    it('should filter by status=PLANNING', async () => {
      ...
      await db.collection(COLLECTIONS.TRIPS).doc(planningTrip.id).set(planningTrip)
