...
describe("Feed Operations", () => {
	...
	describe("getPublicFeed", () => {
		it("should return only public trips", async () => {
			...
			expect(result.trips.every((trip) => trip.access_level === "public")).toBe(
				true,
			);
			expect(result.trips.some((trip) => trip.id === publicTrip1.id)).toBe(
				true,
			);
			expect(result.trips.some((trip) => trip.id === publicTrip2.id)).toBe(
				true,
			);
			expect(result.trips.some((trip) => trip.id === publicTrip3.id)).toBe(
				true,
			);
		});
		...
		it("should exclude private trips", async () => {
			...
			expect(result.trips.some((trip) => trip.id === privateTrip.id)).toBe(
				false,
			);
		});
	...
	describe("getTrendingFeed", () => {
		...
		it("should exclude trips without trending_score", async () => {
			...
			expect(result.trips.some((trip) => trip.id === tripWithoutScore.id)).toBe(
				false,
			);
		});
	...
	describe("getFollowingFeed", () => {
		...
		it("should return only public trips from followed users", async () => {
			...
			expect(result.trips.every((trip) => trip.user_id === otherUserId)).toBe(
				true,
			);
		});
		...
		it("should exclude trips from non-followed users", async () => {
			...
			expect(
				result.trips.some((trip) => trip.id === tripFromNonFollowed.id),
			).toBe(false);
		});
		...
		it("should support pagination with cursor", async () => {
			...
			if (firstPage.trips.length === 2) {
				...
				// 次のページを取得
				if (firstPage.nextCursor) {
					const secondPage = await getFollowingFeed(
						userId,
						2,
