...
export class RouteOptimizer {
  ...
  public async calculateRouteOptimized(
    ...
    const cacheKey: RouteCacheKey = {
      ...
      waypoints: request.waypoints.map(wp => typeof wp === 'string' ? wp : `${wp.lat},${wp.lng}`),
 ...
 * ルート計算のコスト見積もり（改善版）
 */
export const estimateRouteCost = async (waypointCount: number): Promise<{
  ...
  try {
    const response = await fetch(`/api/route-optimization?waypoints=${waypointCount}`)
    if (response.ok) {
...
export const compareRouteOptions = async (
  ...
  try {
    const options = [
      { travelMode: 'DRIVING' as const, avoidHighways: false, avoidTolls: false },
      { travelMode: 'DRIVING' as const, avoidHighways: true, avoidTolls: false },
      { travelMode: 'DRIVING' as const, avoidHighways: false, avoidTolls: true },
    ...
    const results = await Promise.allSettled(
      options.map(option => optimizeWaypoints(waypoints, origin, destination, option))
    )
    ...
    const fastestRoute = validResults.reduce((fastest, current) => 
      current.totalDuration.seconds < fastest.totalDuration.seconds ? current : fastest
    )

    // 最短ルート
    const shortestRoute = validResults.reduce((shortest, current) => 
      current.totalDistance.meters < shortest.totalDistance.meters ? current : shortest
    )

    // 最安ルート（APIコストベース）
    const cheapestRoute = validResults.reduce((cheapest, current) => 
      current.costEstimate.estimatedCost < cheapest.costEstimate.estimatedCost ? current : cheapest
    )

    const recommendations: string[] = []
    if (fastestRoute.totalDuration.seconds < shortestRoute.totalDuration.seconds * 0.8) {
      recommendations.push('最速ルートが大幅に時間短縮できます')
    }
    if (shortestRoute.totalDistance.meters < fastestRoute.totalDistance.meters * 0.8) {
      recommendations.push('最短ルートが大幅に距離短縮できます')
    }
    if (cheapestRoute.costEstimate.estimatedCost < fastestRoute.costEstimate.estimatedCost * 0.5) {
 ...
 * 料金削減の提案（改善版）
 */
export const getCostOptimizationSuggestions = async (waypointCount: number): Promise<string[]> => {
  try {
