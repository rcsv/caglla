...
function decodeCursor(
	cursor: string,
): { tripId: string; sortValue: number } | null {
	try {
...
export async function getAccessibleTrips(
	...
	if (cursorDate && cursorTripId) {
		...
		const cursorDoc = await firestore
			...
			.get();
		if (cursorDoc.exists) {
			ownedTripsQuery = ownedTripsQuery.startAfter(
				cursorDoc,
			) as typeof ownedTripsQuery;
		}
	}
	...
	if (includeShared) {
		...
		const sharedTripIds = sharedTripUsersSnapshot.docs.map(
			(doc) => doc.data().trip_id as string,
		);
		...
		if (sharedTripIds.length > 0) {
			...
			const sharedTripsPromises = batches.map(async (batch) => {
				const tripDocs = await Promise.all(
					batch.map((tripId) =>
						firestore.collection(COLLECTIONS.TRIPS).doc(tripId).get(),
					),
				);
	...
	if (accessLevel) {
		if (accessLevel === "private") {
			allTrips = allTrips.filter(
				(trip) =>
					trip.access_level === "private" || trip.access_level === "shared",
			);
	...
	const nextCursor =
		hasMore && trips.length > 0
			? (() => {
					const lastCreatedAt = toDateOrNull(
						trips[trips.length - 1].created_at,
					);
