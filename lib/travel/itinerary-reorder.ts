...
export const reorderItineraries = async (
	...
	try {
		...
		if (
			...
			error.message.includes("server update skipped")
		) {
			return {
				success: true,
...
export const applyOptimizedOrder = async (
	...
	try {
		...
		logger.debug(
			"Valid itineraries:",
			validItineraries.map((it) => ({ id: it.id, name: it.place_data?.name })),
		);

		if (validItineraries.length !== optimizedOrder.length) {
			throw new Error(
				`Optimized order length (${optimizedOrder.length}) does not match valid itineraries count (${validItineraries.length})`,
			);
		}

		// 最適化された順序でitineraryを並び替え
		const reorderedItineraries = optimizedOrder.map(
			(index) => validItineraries[index],
		);

		logger.debug(
			"Reordered itineraries:",
			reorderedItineraries.map((it) => ({
				id: it.id,
