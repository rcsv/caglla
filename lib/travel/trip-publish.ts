 ...
 * ```
 */
export async function publishTrip(tripSlug: string, slug?: string): Promise<Trip> {
  try {
    ...
    const response = await makeAuthenticatedRequest(`/api/trip/${tripSlug}/publish`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(error.error || `Failed to publish trip: ${response.status}`)
    }

    const result = await response.json()
    logger.debug('Trip published successfully', { tripId: result.trip?.id || tripSlug })
    return result.trip
  } catch (error) {
...
export async function unpublishTrip(tripSlug: string): Promise<Trip> {
  try {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ error: 'Unknown error' }))
      throw new Error(error.error || `Failed to unpublish trip: ${response.status}`)
    }

    const result = await response.json()
    logger.debug('Trip unpublished successfully', { tripId: result.trip?.id || tripSlug })
    return result.trip
  } catch (error) {
