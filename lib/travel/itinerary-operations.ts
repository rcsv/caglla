...
export async function createItinerary(
	...
	try {
		logger.debug("Creating itinerary", {
			day_id: data.day_id,
			title: data.title,
		});
		...
		const body: Record<string, any> = {
			...
			...(data.insert_after_index !== undefined && {
		...
		if (!response.ok) {
			...
			throw new Error(
				error.error || `Failed to create itinerary: ${response.status}`,
			);
		}

		const itinerary = await response.json();
		logger.debug("Itinerary created successfully", {
...
export async function updateItinerary(
	...
	try {
		...
		if (data.cost_currency !== undefined)
			body.cost_currency = data.cost_currency;
		...
		if (!response.ok) {
			const error = await response
				...
				.catch(() => ({ error: "Unknown error" }));
			throw new Error(
				error.error || `Failed to update itinerary: ${response.status}`,
			);
		}

		const itinerary = await response.json();
		logger.debug("Itinerary updated successfully", {
			itineraryId: itinerary.id,
		});
		return itinerary;
	} catch (error) {
	...
	try {
		...
		if (!response.ok) {
			const error = await response
				...
				.catch(() => ({ error: "Unknown error" }));
			throw new Error(
				error.error || `Failed to delete itinerary: ${response.status}`,
			);
		}
	...
	try {
		...
		if (!response.ok) {
			const error = await response
				...
				.catch(() => ({ error: "Unknown error" }));
			throw new Error(
				error.error || `Failed to reorder itineraries: ${response.status}`,
			);
		}
