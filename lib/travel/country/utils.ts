...
import {
	getCountryInfo,
	getCountryNameJa as getCountryNameJaFromFlags,
...
function extractCountryFromEnglishAddress(
	...
	if (directCode) {
		...
		return {
			countryCode: directCode.toLowerCase(),
			countryName: info?.name || token,
		};
	}
...
export async function extractCountryFromAddress(
	...
	try {
		...
		const geocodingResults =
			await geocodingApiHelpers.geocodeAddress(formattedAddress);
...
export async function groupTripsByCountry(
	...
	for (const trip of trips) {
		...
		logger.debug(
			`Final country info: ${countryCode} - ${countryName} (${countryNameJa})`,
		);
