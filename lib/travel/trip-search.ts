...
export async function getRecommendedTrips(
	...
	try {
		...
		const response = await makeAuthenticatedRequest(
			`/api/trips/recommended?limit=${limit}`,
			{
				method: "GET",
			},
		);
		...
		if (!response.ok) {
			...
			throw new Error(
				error.error || `Failed to get recommended trips: ${response.status}`,
			);
		}

		const result = await response.json();
		logger.debug("Recommended trips retrieved successfully", {
...
export async function searchTrips(
	...
	try {
		...
		const response = await makeAuthenticatedRequest(
			`/api/trips/search?${params.toString()}`,
			{
				method: "GET",
			},
		);
		...
		if (!response.ok) {
			const error = await response
				...
				.catch(() => ({ error: "Unknown error" }));
			throw new Error(
				error.error || `Failed to search trips: ${response.status}`,
			);
		}
