...
import {
	convertStandardDates,
	toDateOrNull,
} from "@/lib/firebase/timestamp-utils";
import { getFallbackLanguages, getUserLanguage } from "@/lib/utils/language";
...
export async function getTripBySlug(
	...
	try {
		...
		const destinationPlaceId: string | undefined =
			tripData.destination_place_id;
		if (destinationPlaceId) {
			try {
				const preferred: SupportedLanguage =
					getUserLanguage() as SupportedLanguage;
				const fallbacks = getFallbackLanguages(preferred);

				let resolved: PlacesCache | null = null;
				for (const lang of fallbacks) {
					const cacheKey = `${destinationPlaceId}_${lang}`;
					try {
						const destDoc = await getDoc(
							doc(db, COLLECTIONS.PLACES_CACHE, cacheKey),
						);
						if (destDoc.exists()) {
							resolved = destDoc.data() as PlacesCache;
							break;
						}
					} catch {}
				}

				// レガシー形式のフォールバック（place_id のみ）
				if (!resolved) {
					try {
						const legacyDoc = await getDoc(
							doc(db, COLLECTIONS.PLACES_CACHE, destinationPlaceId),
						);
						if (legacyDoc.exists()) {
							resolved = legacyDoc.data() as PlacesCache;
						}
					} catch {}
				}
				...
				if (resolved) {
					...
					const fetched = await placesCacheManager.fetchAndCachePlace(
						destinationPlaceId,
						preferred,
					);
					if (fetched) {
		...
		const daysWithItineraries = await Promise.all(
			days.map(async (day) => {
				...
				// デバッグ用ログ
				logger.debug(
					`Day ${day.id} itineraries sort_numbers:`,
					itinerariesSnapshot.docs.map((doc) => ({
						id: doc.id,
				...
				const itineraries = (
					await Promise.all(
						itinerariesSnapshot.docs.map(async (docSnap) => {
							...
							if (data.place_id) {
								try {
									...
									if (!placesCache) {
										try {
											const legacyDoc = await getDoc(
												doc(db, COLLECTIONS.PLACES_CACHE, data.place_id),
											);
											if (legacyDoc.exists()) {
									...
									if (placesCache) {
										...
										logger.debug(
											`Resolved place_data for "${data.title}" from cache`,
										);
									} else {
										logger.debug(
											`PlacesCache not found for place_id: ${data.place_id}`,
										);
										if (data.place_data) {
											// キャッシュに無い場合は既存の place_data を使用（後方互換）
											itineraryBase.place_data = data.place_data;
											logger.debug(
												`Using fallback place_data for "${data.title}"`,
											);
										} else {
											// フォールバックもない場合、APIから取得してキャッシュ
											logger.info(
												`Fetching place_data from API for "${data.title}"`,
											);
											try {
												const fetchedPlaceData =
													await placesCacheManager.fetchAndCachePlace(
														data.place_id,
														preferred,
													);
												if (fetchedPlaceData) {
													...
													itineraryBase.place_data = {
														...
														formatted_address:
															fetchedPlaceData.formatted_address,
														geometry: fetchedPlaceData.geometry,
														address_components:
															fetchedPlaceData.address_components,
														photos: fetchedPlaceData.photos,
														rating: fetchedPlaceData.rating,
														user_ratings_total:
															fetchedPlaceData.user_ratings_total,
														price_level: fetchedPlaceData.price_level,
														types: fetchedPlaceData.types,
														opening_hours: fetchedPlaceData.opening_hours,
														international_phone_number:
															fetchedPlaceData.international_phone_number,
														website: fetchedPlaceData.website,
														editorial_summary:
															fetchedPlaceData.editorial_summary,
													};
													logger.info(
														`Fetched and cached place_data for "${data.title}"`,
													);
												} else {
													logger.warn(
														`Failed to fetch place_data for "${data.title}"`,
													);
												}
											} catch (error) {
												logger.error(
													`Error fetching place_data for "${data.title}":`,
													error,
												);
											}
										}
									...
									logger.error(
										`Failed to resolve place_data for "${data.title}":`,
										error,
									);
					...
					(a: Itinerary, b: Itinerary) =>
						(a.sort_number || 0) - (b.sort_number || 0),
				); // sort_number順でソート
 ...
 */
export async function getTripBySlugs(
	userSlug: string,
	tripSlug: string,
): Promise<Trip | null> {
 ...
 */
export async function getSlugsFromTripId(
	tripId: string,
): Promise<{ userSlug: string; tripSlug: string } | null> {
	const db = getFirestore();
