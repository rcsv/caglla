...
export async function updateTrip(
	...
	try {
		...
		if (data.destinationPlaceId !== undefined)
			body.destinationPlaceId = data.destinationPlaceId;
		...
		if (!response.ok) {
			...
			throw new Error(
				error.error || `Failed to update trip: ${response.status}`,
			);
		}
	...
	try {
		...
		if (!response.ok) {
			const error = await response
				...
				.catch(() => ({ error: "Unknown error" }));
			throw new Error(
				error.error || `Failed to delete trip: ${response.status}`,
			);
		}
