...
export async function getTripServer(
	...
	try {
				...
				const itineraries = await Promise.all(
					rawItineraries.map(async (it: any) => {
						if (it.place_id) {
							...
							it.place_data &&
							typeof it.place_data === "object" &&
							"toDate" in it.place_data
						) {
							// Firestore Timestampが残っている場合は変換
							logger.debug("Converting Firestore Timestamp in place_data", {
								itineraryId: it.id,
							});
						}
		...
		const tripForClient: TripWithDestination = {
			...
			ical_last_accessed_at: toDate(tripWithDest.ical_last_accessed_at) as
				| FirestoreDate
				| undefined,
		};
