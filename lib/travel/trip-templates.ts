	...
	const tripsWithCreator = await Promise.all(
		trips.map(async (trip) => {
			...
			try {
				...
				logger.error("Error fetching creator for template trip", error, {
					tripId: trip.id,
					userId: trip.user_id,
				});
			}
...
export async function getMySharedTrips(
	...
	const nextCursor =
		hasMore && trips.length > 0
			? (() => {
					const lastUpdatedAt = toDateOrNull(
						trips[trips.length - 1].updated_at,
					);
					return lastUpdatedAt
						? encodeCursor(trips[trips.length - 1].id, lastUpdatedAt)
						: undefined;
				})()
			: undefined;
