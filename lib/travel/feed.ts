...
export async function getFollowingFeed(
	...
	for (const followingId of followingIds) {
		try {
			...
			logger.warn("Failed to get user info for following_id", {
				followingId,
				error,
			});
	...
	const tripsWithCreator = await Promise.all(
		trips.map(async (trip) => {
			...
			try {
				...
				logger.error("Error fetching creator for following feed trip", error, {
					tripId: trip.id,
					userId: trip.user_id,
				});
			}
