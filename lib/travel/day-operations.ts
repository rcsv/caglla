...
export async function createDay(tripSlug: string): Promise<Day> {
	try {
		...
		if (!response.ok) {
			...
			throw new Error(
				error.error || `Failed to create day: ${response.status}`,
			);
		}
	...
	try {
		...
		if (!response.ok) {
			const error = await response
				...
				.catch(() => ({ error: "Unknown error" }));
			throw new Error(
				error.error || `Failed to update day: ${response.status}`,
			);
		}
	...
	try {
		...
		if (!response.ok) {
			const error = await response
				...
				.catch(() => ({ error: "Unknown error" }));
			throw new Error(
				error.error || `Failed to delete day: ${response.status}`,
			);
		}
	...
	try {
		...
		if (!response.ok) {
			const error = await response
				...
				.catch(() => ({ error: "Unknown error" }));
			throw new Error(
				error.error || `Failed to update days for trip: ${response.status}`,
			);
		}
