 ...
 */
export const PlaceDataSchema: z.ZodType<PlaceData> = z.object({
  place_id: z.string(),
  name: z.string(),
...
export const CreateTripSchema = z.object({
  ...
  dayCount: z.number().int().positive('Day count must be positive').optional(),
  day_count: z.number().int().positive('Day count must be positive').optional(),
  likesCount: z.number().int().min(0, 'Likes count must be non-negative').optional(),
  likes_count: z.number().int().min(0, 'Likes count must be non-negative').optional(),
  imageUrl: z.string().url({ message: 'Invalid image URL format' }).optional(),
  image_url: z.string().url({ message: 'Invalid image URL format' }).optional(),
  defaultCurrency: z.string().length(3, 'Currency code must be 3 characters (e.g., USD, JPY)').optional(),
  default_currency: z.string().length(3, 'Currency code must be 3 characters (e.g., USD, JPY)').optional()
}).refine(
  (data) => {
 ...
 */
export const UpdateTripSchema: z.ZodTypeAny = CreateTripSchema.partial() as z.ZodTypeAny
