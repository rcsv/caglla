...
const SupportedLanguageSchema = z.enum([
  'en', 'ja', 'ko', 'zh-CN', 'zh-TW', 'es', 'fr', 'de', 'it', 'pt', 'ru', 'ar', 'hi', 'th', 'vi'
] as const).default(DEFAULT_LANGUAGE)
...
export const PlaceNearbySchema = z.object({
  ...
  radius: z.number().int().positive('Radius must be a positive number').optional()
})
