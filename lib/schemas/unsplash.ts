...
export const UnsplashQuerySchema = z.object({
  ...
  count: z.coerce.number().int().positive().max(10, 'Count cannot exceed 10').default(1).optional()
})
...
export const UnsplashBodySchema = z.object({
  ...
  count: z.number().int().positive().max(10, 'Count cannot exceed 10').default(1).optional()
})
