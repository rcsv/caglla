 ...
 */
export const InsertItinerarySchema = z
	.object({
		day_id: z.string().min(1, "Day ID is required"),
		place_id: z.string().optional(),
		place_data: PlaceDataSchema.optional(),
...
export const UpdateItinerarySchema = z
	.object({
		...
		sort_number: z
			.number()
			.int()
			.min(1, "Sort number must be a positive integer")
			.optional(),
		// 通常の更新リクエスト用フィールド
	...
	.refine(
		(data) => {
			...
			const isReorder =
				data.day_id !== undefined && data.sort_number !== undefined;
		...
		{
			message:
				"Either day_id and sort_number (for reorder) or at least one update field must be provided",
			path: [],
		},
	);
