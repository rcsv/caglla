 ...
 * 飛行機予約のスキーマ
 */
export const FlightReservationSchema = z
	.object({
		...
		departure_at: z
			.string()
			.datetime({ message: "Invalid departure datetime format" }),
		arrival_at: z
			.string()
			.datetime({ message: "Invalid arrival datetime format" }),
		...
		reservation_url: z
			...
			.refine(isAllowedReservationUrl, {
				message:
					"Invalid reservation URL. Only HTTPS URLs from allowed sites are permitted",
			})
			.optional(),
 ...
 */
export const OtherReservationSchema = z
	.object({
		type: z.enum(["rental_car", "hotel", "dining", "other"]),
		start_date: z
			.string()
			.datetime({ message: "Invalid start datetime format" }),
 ...
 */
const ClientFlightReservationSchema = z
	.object({
		type: z.literal("flight"),
	...
	.refine(
		(data) => {
			if (data.departure_at && data.arrival_at) {
				const departure =
					data.departure_at instanceof Date
						? data.departure_at
						: typeof data.departure_at === "object" && data.departure_at?.toDate
							? data.departure_at.toDate()
							: new Date(data.departure_at);
				const arrival =
					data.arrival_at instanceof Date
						? data.arrival_at
						: typeof data.arrival_at === "object" && data.arrival_at?.toDate
							? data.arrival_at.toDate()
							: new Date(data.arrival_at);
				return departure.getTime() < arrival.getTime();
			}
			return true;
		},
 ...
 */
const ClientOtherReservationSchema = z
	.object({
		type: z.enum(["rental_car", "hotel", "dining", "other"]),
			...
			if (data.start_date && data.end_date) {
				const start =
					data.start_date instanceof Date
						? data.start_date
						: typeof data.start_date === "object" && data.start_date?.toDate
							? data.start_date.toDate()
							: new Date(data.start_date);
				const end =
					data.end_date instanceof Date
						? data.end_date
						: typeof data.end_date === "object" && data.end_date?.toDate
 ...
 */
export const ClientReservationInfoSchema = z
	.object({
