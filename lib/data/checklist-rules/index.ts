...
export function getRulesByCategory(
	...
	switch (category.toLowerCase()) {
		case "transportation":
			return TRANSPORTATION_RULES;
		case "shopping":
			return SHOPPING_RULES;
		case "accommodation":
			return ACCOMMODATION_RULES;
		case "adventure":
			return ADVENTURE_RULES;
		case "entertainment":
			return ENTERTAINMENT_RULES;
		case "culture":
			return CULTURE_RULES;
		case "wellness":
			return WELLNESS_RULES;
		case "service":
 ...
 */
export function getChecklistRules(
	secondaryCategoryId: string,
): ChecklistGenerationRule[] {
	return CHECKLIST_RULES.filter(
		(rule) => rule.secondaryCategory === secondaryCategoryId,
	);
}
