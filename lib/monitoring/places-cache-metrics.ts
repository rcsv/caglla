...
class PlacesCacheMetrics {
  ...
  private flush() {
    ...
    const apiErrorRate = apiTotal > 0 
      ? (this.apiErrors / apiTotal * 100).toFixed(2) 
      : '0.00'
    ...
    // アラート条件チェック
    this.checkAlerts(parseFloat(hitRatio), parseFloat(apiErrorRate), parseFloat(avgResponseTime))
  }

  /**
   * アラート条件をチェック
   */
  private checkAlerts(hitRatio: number, apiErrorRate: number, avgResponseTime: number) {
    // キャッシュヒット率が30%以下
    if (this.hits + this.misses >= 100 && hitRatio < 30) {
      logger.warn('⚠️  ALERT: Low cache hit ratio', { hitRatio: `${hitRatio}%` })
    }

    // APIエラー率が5%以上
    if (this.apiSuccess + this.apiErrors >= 20 && apiErrorRate > 5) {
      logger.error('🚨 ALERT: High API error rate', { apiErrorRate: `${apiErrorRate}%` })
    }

    // 平均レスポンス時間が1秒以上
    if (this.responseTimes.length >= 10 && avgResponseTime > 1000) {
      logger.warn('⚠️  ALERT: Slow response time', { avgResponseTime: `${avgResponseTime}ms` })
    }
  }
