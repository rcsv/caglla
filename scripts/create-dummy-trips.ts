	...
	try {
		...
		const categories = [
			...
			},
			{
				...
				data: [
					{
						title: "2023年12月の沖縄リゾート",
						destination: "沖縄県那覇市",
						days: 5,
						offset: -330,
					},
					{
						title: "2024年1月の京都・奈良観光",
						destination: "京都府",
						days: 4,
						offset: -300,
					},
					{
						title: "2024年2月の北海道スキー旅行",
						destination: "北海道札幌市",
						days: 6,
						offset: -270,
					},
					{
						title: "2024年3月の東京ディズニー",
						destination: "千葉県浦安市",
						days: 3,
						offset: -240,
					},
					{
						title: "2024年4月の大阪・神戸グルメ旅",
						destination: "大阪府",
						days: 4,
						offset: -210,
					},
		...
		for (const user of USERS) {
			...
			for (const category of categories) {
				...
				for (let i = 0; i < category.data.length; i++) {
					...
					const trip = await category.createFn(
						user,
						tripData,
						i,
						category.count,
		...
		console.log("Summary:", {
			...
			byType: {
				draft_guide: createdTrips.filter((t) => t.type === "draft_guide")
					.length,
				published_guide: createdTrips.filter(
					(t) => t.type === "published_guide",
				).length,
				ongoing: createdTrips.filter((t) => t.type === "ongoing").length,
				upcoming: createdTrips.filter((t) => t.type === "upcoming").length,
				private_not_shared: createdTrips.filter(
					(t) => t.type === "private_not_shared",
				).length,
				private_shared: createdTrips.filter((t) => t.type === "private_shared")
					.length,
			},
			byUser: USERS.map((u) => ({
				userId: u.id,
