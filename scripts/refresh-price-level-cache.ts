	...
	try {
		...
		for (const doc of snapshot.docs) {
			...
			if (data.price_level !== undefined && data.price_level !== null) {
				...
				logger.debug(
					`✅ Skipping ${data.name} - already has price_level: ${data.price_level}`,
				);
				continue;
			}
			...
			try {
				...
				if (apiData.priceLevel) {
					...
					if (priceLevel >= 0) {
						...
						logger.info(
							`   ✅ Updated price_level: ${priceLevel} (${"$".repeat(priceLevel + 1)})`,
						);
						stats.updated++;
					}
				} else {
					logger.info(
						`   ℹ️  No price_level in API response (observation/landmark)`,
					);
		...
		console.log(
			`Still no price_level:          ${stats.stillNoPriceLevel} (observation/landmark)`,
		);
