	...
	try {
		...
		snapshot.docs.forEach((doc: any) => {
			...
			if (data.price_level !== undefined && data.price_level !== null) {
				...
				if (level >= 0 && level <= 4) {
					stats.priceLevelDistribution[
						level as keyof typeof stats.priceLevelDistribution
					]++;
				}
		...
		console.log(
			`  With price_level:          ${stats.withPriceLevel} (${((stats.withPriceLevel / stats.totalDocuments) * 100).toFixed(2)}%)`,
		);
		console.log(
			`  Without price_level:       ${stats.withoutPriceLevel} (${((stats.withoutPriceLevel / stats.totalDocuments) * 100).toFixed(2)}%)`,
		);

		console.log("\n💰 Price Level Distribution:");
		console.log(
			`  Level 0 (Free):            ${stats.priceLevelDistribution[0]} places`,
		);
		console.log(
			`  Level 1 (Inexpensive):     ${stats.priceLevelDistribution[1]} places`,
		);
		console.log(
			`  Level 2 (Moderate):        ${stats.priceLevelDistribution[2]} places`,
		);
		console.log(
			`  Level 3 (Expensive):       ${stats.priceLevelDistribution[3]} places`,
		);
		console.log(
			`  Level 4 (Very Expensive):  ${stats.priceLevelDistribution[4]} places`,
		);

		console.log("\n🚫 Missing Data Flags:");
		console.log(
			`  Documents with missing_data_flags:        ${stats.missingDataFlagsCount} (${((stats.missingDataFlagsCount / stats.totalDocuments) * 100).toFixed(2)}%)`,
		);
		console.log(
			`  Persistently missing price_level:         ${stats.persistentlyMissingPriceLevel} (${((stats.persistentlyMissingPriceLevel / stats.totalDocuments) * 100).toFixed(2)}%)`,
		);
		...
		stats.samplePlacesWithPriceLevel.forEach((place, index) => {
			...
			console.log(
				`     price_level: ${place.price_level} (${"$".repeat(place.price_level + 1)})`,
			);
		});
		...
		stats.samplePlacesWithoutPriceLevel.forEach((place, index) => {
			...
			console.log(`     place_id: ${place.place_id}`);
			console.log(
				`     has_missing_flag: ${place.has_missing_flag ? "✅ Yes (永続的欠損)" : "❌ No"}`,
			);
		});
