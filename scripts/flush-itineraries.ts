...
async function flushItinerariesData() {
	try {
		...
		logger.debug(
			`📊 Found ${itinerariesSnapshot.docs.length} itineraries to delete`,
		);
		...
		for (let i = 0; i < itinerariesSnapshot.docs.length; i++) {
			...
			if ((i + 1) % batchSize === 0) {
				...
				batchCount++;
				logger.debug(
					`📦 Prepared batch ${batchCount} (${batchSize} documents)`,
				);
			}
		}
		...
		if (currentBatch._mutations.length > 0) {
			...
			batchCount++;
			logger.debug(
				`📦 Prepared final batch ${batchCount} (${currentBatch._mutations.length} documents)`,
			);
		}
		...
		logger.debug(
			`📊 Total deleted: ${itinerariesSnapshot.docs.length} documents`,
		);
	} catch (error) {
