 ...
 * 旧形式キャッシュを新形式に移行
 */
async function migrateCache(options: { dryRun: boolean; limit: number; batchSize: number }) {
  ...
  try {
    ...
    const snapshot = await db.collection(COLLECTIONS.PLACES_CACHE)
      .limit(options.limit)
      .get()
    ...
    for (const doc of snapshot.docs) {
      ...
      // 旧形式かどうかチェック
      const isOldFormat = !data.format_version || data.format_version === '1.0.0'
      ...
      try {
        ...
        // 新ドキュメントを作成
        await db.collection(COLLECTIONS.PLACES_CACHE).doc(newDocId).set(newData)
