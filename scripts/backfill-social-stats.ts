...
function getAdminDb(): Firestore {
  if (!adminDb) {
    try {
      ...
      throw new Error('Firebase Admin SDK is not available. Provide a Firestore instance as the first parameter.')
    }
  }
  return adminDb
}
 ...
 */
export function hasCompleteSocialStats(stats: unknown): stats is TripSocialStats {
...
export function completeSocialStats(partialStats: unknown): TripSocialStats {
  ...
  return {
    ...
    comments_count: typeof stats.comments_count === 'number' ? stats.comments_count : 0,
    shares_count: typeof stats.shares_count === 'number' ? stats.shares_count : 0,
    views_count: typeof stats.views_count === 'number' ? stats.views_count : 0,
    replicas_count: typeof stats.replicas_count === 'number' ? stats.replicas_count : 0,
  }
}
  ...
  try {
    while (true) {
      ...
      if (!dryRun && batchUpdatedCount > 0) {
        await batch.commit()
        logger.info('Batch committed', { batchUpdatedCount, totalUpdated: updatedCount })
      } else if (dryRun && batchUpdatedCount > 0) {
        logger.info('Dry run - would update', { batchUpdatedCount, totalUpdated: updatedCount })
      }
...
async function main() {
  ...
  try {
    const updatedCount = await backfillSocialStats(getAdminDb(), batchSize, dryRun)
