  ...
  try {
    ...
    for (const tripDoc of tripsSnapshot.docs) {
      ...
      try {
        ...
        const userDoc = await adminDb.collection(COLLECTIONS.USERS).doc(currentUserId).get()
        
        if (userDoc.exists) {
          // 既に users コレクションのドキュメントIDの場合はスキップ
          logger.debug(`Trip ${trip.id} already uses user document ID, skipping`, { userId: currentUserId })
          skippedCount++
          continue
        }
        ...
        if (userMappingCache.has(currentUserId)) {
          ...
          const user = await adminUserOperations.getUserByAuthUid(currentUserId)
          if (user) {
        ...
        if (!userDocumentId) {
          logger.warn(`User not found for trip ${trip.id}`, { userId: currentUserId })
          errorCount++
          errors.push({ tripId: trip.id, error: `User not found: ${currentUserId}` })
          continue
        }
        ...
        logger.error(`❌ Error migrating trip ${trip.id}`, { error, userId: currentUserId })
        errorCount++
        errors.push({
          tripId: trip.id,
