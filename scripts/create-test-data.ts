...
async function createTestUser(userData: {
  ...
  logger.info('Creating test user...', { name: userData.name, email: userData.email })

  // 既存ユーザーをチェック
  const existingUser = await adminUserOperations.getUserByAuthUid(userData.authUid)
  if (existingUser) {
...
async function main() {
  try {
    logger.info('🚀 Starting template creation for user: faRoL34yjICfd6v21VMr...')
    ...
    logger.info('✅ User found', { userId: user.id, name: user.name, slug: user.slug })

    // 旅行テンプレートのリスト（public）
    const publicTemplates = [
      { title: '東京3日間観光プラン', destination: '東京都', description: '東京の主要観光スポットを巡る3日間のプラン', dayCount: 3 },
      { title: '京都・奈良2泊3日', destination: '京都府', description: '古都京都と奈良を巡る歴史散策プラン', dayCount: 3 },
      { title: '沖縄リゾート5日間', destination: '沖縄県那覇市', description: '沖縄のビーチと文化を楽しむ5日間プラン', dayCount: 5 },
      { title: '北海道冬の旅4日間', destination: '北海道札幌市', description: '雪景色と温泉を楽しむ冬の北海道プラン', dayCount: 4 },
      { title: '大阪・神戸グルメ旅2日間', destination: '大阪府', description: '関西の名物グルメを巡る2日間プラン', dayCount: 2 },
      { title: '金沢・白川郷3日間', destination: '石川県金沢市', description: '加賀百万石の城下町と世界遺産を巡るプラン', dayCount: 3 },
      { title: '箱根温泉1泊2日', destination: '神奈川県箱根町', description: '箱根の温泉と自然を楽しむリラックスプラン', dayCount: 2 },
      { title: '広島・宮島観光2日間', destination: '広島県広島市', description: '原爆ドームと厳島神社を巡る歴史プラン', dayCount: 2 },
      { title: '福岡・博多グルメ旅3日間', destination: '福岡県福岡市', description: '博多ラーメンと屋台グルメを楽しむプラン', dayCount: 3 },
      { title: '長野・軽井沢避暑地4日間', destination: '長野県軽井沢町', description: '高原の涼しい空気と自然を楽しむプラン', dayCount: 4 },
    ]

    // プライベートテンプレートのリスト（執筆中）
    const privateTemplates = [
      { title: '執筆中：鎌倉・江ノ島1日観光', destination: '神奈川県鎌倉市', description: '鎌倉の寺社と江ノ島を巡る1日プラン（執筆中）', dayCount: 1 },
      { title: '執筆中：伊豆・熱海温泉旅2日間', destination: '静岡県熱海市', description: '伊豆半島の温泉と海を楽しむプラン（執筆中）', dayCount: 2 },
      { title: '執筆中：高知・四万十川3日間', destination: '高知県高知市', description: '四万十川と高知の自然を楽しむプラン（執筆中）', dayCount: 3 },
      { title: '執筆中：熊本・阿蘇2日間', destination: '熊本県熊本市', description: '阿蘇山と熊本城を巡るプラン（執筆中）', dayCount: 2 },
      { title: '執筆中：鹿児島・桜島1泊2日', destination: '鹿児島県鹿児島市', description: '桜島と鹿児島の歴史を楽しむプラン（執筆中）', dayCount: 2 },
    ]

    const createdTrips: Array<{ id: string; title: string; slug?: string; accessLevel: string }> = []
    ...
    for (const template of publicTemplates) {
      ...
      createdTrips.push({ id: trip.id, title: trip.title, slug: trip.slug, accessLevel: 'public' })
      logger.info(`✅ Created public template: ${template.title}`, { tripId: trip.id })
    }
    ...
    for (const template of privateTemplates) {
      ...
      createdTrips.push({ id: trip.id, title: trip.title, slug: trip.slug, accessLevel: 'private' })
      logger.info(`✅ Created private template: ${template.title}`, { tripId: trip.id })
    }
    ...
    logger.info('Created templates:', {
      ...
      publicTemplates: createdTrips.filter(t => t.accessLevel === 'public').length,
      privateTemplates: createdTrips.filter(t => t.accessLevel === 'private').length,
      trips: createdTrips,
    })
