...
async function testPriceLevelAPI(): Promise<void> {
  ...
  for (const testPlace of TEST_PLACES) {
    ...
    try {
      const response = await fetch(
        `https://places.googleapis.com/v1/places/${testPlace.place_id}?languageCode=ja`,
        {
          ...
          headers: {
            ...
            'X-Goog-FieldMask': 'id,displayName,priceLevel,rating,userRatingCount',
            'Accept-Language': 'ja'
          }
        }
      )
      ...
      console.log(`      - displayName: ${data.displayName?.text || data.displayName}`)
      ...
      console.log(`      - userRatingCount: ${data.userRatingCount || 'undefined'}`)
      
      // 変換ロジックのテスト
      if (data.priceLevel) {
        // ❌ 旧ロジック（バグ）
        const oldPriceLevels = ['FREE', 'INEXPENSIVE', 'MODERATE', 'EXPENSIVE', 'VERY_EXPENSIVE']
        const oldIndex = oldPriceLevels.indexOf(data.priceLevel)
        console.log(`      - OLD conversion (buggy): ${oldIndex >= 0 ? oldIndex : 'undefined'} ❌`)
        
        // ✅ 新ロジック（修正済み）
        const newPriceLevels = ['PRICE_LEVEL_FREE', 'PRICE_LEVEL_INEXPENSIVE', 'PRICE_LEVEL_MODERATE', 'PRICE_LEVEL_EXPENSIVE', 'PRICE_LEVEL_VERY_EXPENSIVE']
        const newIndex = newPriceLevels.indexOf(data.priceLevel)
        console.log(`      - NEW conversion (fixed): ${newIndex >= 0 ? newIndex : 'undefined'} ✅`)
        console.log(`      - As dollar signs: ${'$'.repeat((newIndex >= 0 ? newIndex : 0) + 1)}`)
      } else {
