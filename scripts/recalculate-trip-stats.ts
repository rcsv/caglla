...
function getAdminDb(): Firestore {
  if (!adminDb) {
    try {
      ...
      throw new Error('Firebase Admin SDK is not available. Set FIREBASE credentials and try again.')
    }
  }
...
async function countItineraries(db: Firestore, tripId: string): Promise<number> {
...
async function countChecklistItems(db: Firestore, tripId: string): Promise<number> {
...
export async function recalculateTripStats(
  ...
  try {
    while (true) {
      let query = db.collection(COLLECTIONS.TRIPS).orderBy('created_at').limit(batchSize)
      if (lastDoc) {
      ...
      if (!dryRun && batchUpdatedCount > 0) {
        await batch.commit()
        logger.info('Trip stats batch committed', { batchUpdatedCount, totalUpdated: updatedCount })
      } else if (dryRun && batchUpdatedCount > 0) {
        logger.info('Dry run - would update trip stats', { batchUpdatedCount, totalUpdated: updatedCount })
      }
    ...
    logger.info('Trip stats recalculation completed', { totalUpdated: updatedCount, dryRun })
    return updatedCount
  } catch (error) {
...
async function main() {
  ...
  if (dryRun) {
    logger.warn('Running trip stats recalculation in DRY RUN mode - no changes will be made')
  }

  try {
    const updatedCount = await recalculateTripStats(getAdminDb(), batchSize, dryRun)

    if (dryRun) {
      logger.info(`Trip stats dry run completed: ${updatedCount} trips would be updated`)
    } else {
      logger.info(`Trip stats recalculation completed: ${updatedCount} trips updated`)
    }
