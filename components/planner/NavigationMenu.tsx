...
export default function NavigationMenu({
	...
	const templateWithoutDates = Boolean(
	...
	return (
		<div
			className={`bg-white border-r border-gray-200 h-full flex flex-col transition-all duration-200 relative z-30 left-nav-shadow ${
		...
		>
			...
			<div
				className={`border-b border-gray-200 ${isCollapsed ? "p-2" : "p-3"}`}
			>
				{isCollapsed ? (
					...
					{menuSections.map((section) => (
						...
						>
							...
							>
								...
								>
									<span className="text-gray-600 flex items-center justify-center w-6 h-6">
										{section.icon}
									</span>
									{!isCollapsed && (
							...
							{section.isExpandable &&
								...
								section.children && (
									<div
										className={`${isCollapsed ? "" : "mt-1"} space-y-1 ${
											section.id === "itinerary"
												? "flex-1 overflow-y-auto min-h-0 scrollbar-hide bg-white"
												: ""
										}`}
									>
													...
													<div className="flex flex-col items-center space-y-1">
														{item.id.startsWith("day-") ? (
															<>
																...
																<div
																	className={`text-base font-light leading-[1.1] tracking-tight ${getDayColor(
																		trip.days?.find(
																			(d) =>
																				d.id === item.id.replace("day-", ""),
																		) || ({} as Day),
																	)}`}
																>
																	{getDayNumber(
			...
			{/* 下付きメニュー（UserMenu） */}
			<div
				className={`border-t border-gray-200 ${isCollapsed ? "p-1" : "p-2"} relative zidx-left-panel-content bg-white`}
			>
