...
export const HomeHeader: React.FC<HomeHeaderProps> = ({
	...
	useClickOutside(yourTripsRef as React.RefObject<HTMLElement>, () =>
		setYourTripsOpen(false),
	);
	...
	const planBadgeClass = (() => {
		...
		if (n.includes("globetrotter"))
			return `${base} border-purple-200 bg-purple-50 text-purple-600`;
						...
						>
							<div className="text-right leading-tight">
								<div className="flex items-center gap-1.5 justify-end">
									<div className="text-base font-medium text-gray-900 truncate max-w-[120px] sm:max-w-[160px]">
										{userName}
									</div>
									<span
										className="text-lg leading-none"
										title={
											LANGUAGE_NAMES[currentLanguage]?.native || currentLanguage
										}
								...
								<div className="flex justify-end">
									<span
										className={`${planBadgeClass} max-w-[140px] sm:max-w-[160px] truncate`}
									>
										{planName}
									</span>
								</div>
							...
							<span
								className={`inline-flex p-[2px] rounded-full ${avatarBorderClass}`}
							>
								<Image
						...
						{open && (
							<div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg py-1 zidx-popup-menu">
								...
								<button
									...
									className="w-full text-left px-3 py-2 text-base text-gray-700 hover:bg-gray-50"
								>
									{t("header.changePlan")}
								</button>
								<button
									...
									className="w-full text-left px-3 py-2 text-base text-red-600 hover:bg-gray-50"
								>
									{t("header.logout")}
								</button>
							</div>
