	...
	if (inline) {
		return (
			<span className={containerClass} {...rest}>
				<div
					className={`animate-spin rounded-full ${borderColorClass} ${inlineSizeClass(size)}`}
				></div>
				{finalMessage && (
					<span className="text-sm text-gray-600">{finalMessage}</span>
				)}
			</span>
		);
	}
	...
	return (
		<div className={containerClass} {...rest}>
			<div className="text-center">
				<div
					className={`animate-spin rounded-full mx-auto ${borderColorClass} ${sizeClass(size)}`}
				></div>
