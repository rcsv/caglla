...
export default function PlaceSearchInput({
	...
	const [query, setQuery] = useState(
		currentPlace?.name || (arguments[0]?.initialText as string) || "",
	);
	...
	const searchPlaces = useCallback(
		async (searchQuery: string) => {
			...
			try {
				...
				const results = await placesApiHelpers.searchPlaces(
					searchQuery,
					language,
				);
	...
	const handlePlaceSelect = async (place: PlaceSearchResult) => {
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : t("placeSearch.detailsFailed");
		...
		<div className="relative" ref={containerRef}>
			<div className="relative">
				<input
					...
					disabled={disabled}
					className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-100 disabled:cursor-not-allowed bg-white text-gray-800"
				...
				query.length >= 2 && (
					<div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg">
						<div className="px-4 py-3 text-center text-gray-500 text-sm">
							...
							<div className="mt-2 text-xs text-gray-400">または手動で入力</div>
						</div>
					</div>
				)}
		</div>
