...
export default function QuickPlanModal({ isOpen, onClose }: QuickPlanModalProps) {
  ...
  const loadTemplates = async () => {
    setIsLoading(true)
    try {
      const response = await makeAuthenticatedRequest<{ success: boolean; data: Template[] }>(
        '/api/templates'
      )
  ...
  const handleSelectTemplate = (template: Template) => {
    ...
    if (template.destination) {
      setFormData(prev => ({ ...prev, destination: template.destination || '' }))
    }
  }
  ...
  const handleCreatePlan = async () => {
    ...
    try {
      const response = await makeAuthenticatedRequest<{ success: boolean; data: { trip: Trip } }>(
        '/api/templates',
        {
          method: 'POST',
          body: JSON.stringify({
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Guideから素早くプランを作成</p>
          </div>
          <button
            onClick={onClose}
        ...
        {selectedTemplate && (
          <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-200 bg-gray-50">
            ...
            <button
              ...
              disabled={isCreating || !formData.destination.trim() || !formData.startDate || !formData.endDate}
