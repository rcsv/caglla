...
export default function ImageGalleryModal({
	...
	const [mainImageFallbacks, setMainImageFallbacks] = useState<
			...
			const imagePromises = images.map(async (photo) => {
				...
				return await getCachedPlaceImage(
					...
					googlePhotoUrl,
					{
						...
						height: Math.round(
							maxWidth *
								((photo.height ?? maxWidth) / (photo.width ?? maxWidth)),
						),
						quality: 85,
					},
				);
	...
	const mainImageSrc =
		cachedImage && !mainImageFallbacks[currentIndex]
			? cachedImage.url
			: placesApiHelpers.getPhotoUrl(
					currentImage.photo_reference,
					Math.min(currentImage.width ?? 1600, 1600),
				);
