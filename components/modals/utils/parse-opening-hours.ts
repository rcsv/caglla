...
export function parseOpeningHours(
	...
	const closedKeywords =
		language === "ja" ? ["定休日", "休業日"] : ["closed", "Closed"];
	...
	if (todayText) {
		...
		if (open24hKeywords.some((keyword) => todayText.includes(keyword))) {
			...
			const parsedRanges = timeRanges
				...
				.filter(
					(range): range is { open: string; close: string } => range !== null,
				);

			// 現在時刻がどの営業時間内にあるかチェック
			isOpen = parsedRanges.some(
				(range) => currentTime >= range.open && currentTime <= range.close,
			);

			// 営業時間表示用文字列を生成
			if (parsedRanges.length > 0) {
				currentHours = parsedRanges
	...
	if (
		!isOpen &&
		todayText &&
		!closedKeywords.some((keyword) => todayText.includes(keyword))
	) {
		...
		for (const range of parsedRanges) {
			...
			if (
				currentTimeInMinutes >= openTimeInMinutes - 30 &&
