...
export default function AddScheduleModal({
	...
	const handleSelectPlace = async (place: PlaceSearchResult) => {
		...
		try {
			logger.debug(
				`AddScheduleModal: insertAfterIndex=${insertAfterIndex}, dayId=${dayId}`,
			);
			...
			if (insertAfterIndex !== undefined) {
				...
				logger.debug(
					`Using insert API with insert_after_index=${insertAfterIndex + 1} (converted from 0-based ${insertAfterIndex})`,
				);
			} else {
				...
				<div className="space-y-4">
					...
					<div>
						...
						<div className="flex space-x-2">
							...
							>
								{isSearching
									? t("addScheduleModal.searching")
									: t("addScheduleModal.searchButton")}
							</button>
						</div>
					</div>
					...
					{query && searchResults.length === 0 && !isSearching && (
						<div className="text-center py-4 text-gray-500">
							...
							<p className="text-sm mt-1">
								{t("addScheduleModal.tryDifferentKeyword")}
							</p>
						</div>
					)}
				</div>
