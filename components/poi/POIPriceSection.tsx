				...
				<div className="flex items-center space-x-1.5">
					<div className="flex items-center">
						{starValues.map((value) => (
							<svg
								key={`star-aggregated-${value}`}
								className={`w-3.5 h-3.5 ${
									value <= Math.floor(aggregatedRating.averageRating)
										? "text-yellow-400"
										: "text-gray-300"
								}`}
								fill="currentColor"
				...
				placeRating && (
					<div className="flex items-center space-x-1.5">
						<div className="flex items-center">
							{starValues.map((value) => (
								<svg
									key={`star-place-rating-${value}`}
									className={`w-3.5 h-3.5 ${
										value <= Math.floor(placeRating)
											? "text-yellow-400"
											: "text-gray-300"
									}`}
									fill="currentColor"
