 ...
 */
export function DeleteGuideModal({ trip, isOpen, onClose, onSuccess }: DeleteGuideModalProps) {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await makeAuthenticatedRequest(`/api/trip/${trip.slug || trip.id}`, {
        method: 'DELETE',
      })
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                ...
                <div className="mt-2">
                  <p className="text-sm text-gray-500">
                    {t('tripGuide.modals.delete.message', 'この操作は取り消せません。ガイドとそのすべてのデータが永久に削除されます。')}
                  </p>
                  <p className="text-sm font-medium text-gray-900 mt-2">
                    {trip.title || t('tripGuide.modals.delete.untitled', 'Untitled Guide')}
                  </p>
                </div>
                {error && (
