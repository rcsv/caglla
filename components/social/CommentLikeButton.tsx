...
export default function CommentLikeButton({
  ...
  useEffect(() => {
    ...
    const fetchLikeState = async () => {
      try {
        const response = await makeAuthenticatedRequest(`/api/trip/${tripSlug}/comments/${commentId}/likes`)
        if (response.ok) {
      ...
      if (!response.ok) {
        ...
        const errorMessage = 
          typeof errorData.error === 'string'
            ? errorData.error
            : errorData.error?.message || errorData.message || `Failed to ${nextLiked ? 'like' : 'unlike'} comment`
        throw new Error(errorMessage)
      }

      const data = await response.json()
      const finalLiked = data.liked ?? nextLiked
      const finalCount = typeof data.likesCount === 'number' ? data.likesCount : Number(data.likesCount) || nextCount
