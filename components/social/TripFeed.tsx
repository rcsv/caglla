	...
	const fetchFeed = useCallback(
		async (cursor?: string, append: boolean = false) => {
			try {
				...
				if (feedType === "following") {
					...
					try {
						const response = await makeAuthenticatedRequest(
							getFeedEndpoint("following", cursor),
						);
						if (response.ok) {
							...
							throw new Error(
								`Failed to fetch following feed: ${response.status}`,
							);
						}
					} catch (apiErr) {
						logger.warn(
							`API error for following feed, using mock data:`,
							apiErr,
						);
						// APIエラー時はモックデータを使用
						data = {
					...
					try {
						...
						logger.warn(
							`API error for ${feedType} feed, using mock data:`,
							apiErr,
						);
						// APIエラー時はモックデータを使用
				...
				const mockCount =
					feedType === "trending" ? 8 : feedType === "following" ? 5 : 10;
				const mockData = {
					trips: generateMockTrips(mockCount),
					nextCursor: undefined,
				};
		...
		return (
			...
			>
				...
				<div className="relative flex h-full flex-col justify-between p-5 text-white">
					...
					<div>
						<h3 className="text-2xl font-semibold leading-snug line-clamp-2 drop-shadow-md">
							{trip.title}
						</h3>
						{trip.destination && (
