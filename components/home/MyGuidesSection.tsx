...
export function MyGuidesSection({ trips, loading, onRefresh }: MyGuidesSectionProps) {
  ...
  const getAccessLevelColor = (accessLevel?: string) => {
    if (accessLevel === 'public') return 'text-green-600 bg-green-50 border-green-200'
    if (accessLevel === 'unlisted') return 'text-blue-600 bg-blue-50 border-blue-200'
    ...
    <section className="bg-white rounded-sm shadow-sm border border-gray-200 p-4">
      ...
      {!loading && !hasTrips && (
        <div className="text-center py-4 text-gray-500">
          <Icon icon="mdi:book-outline" className="h-8 w-8 mx-auto mb-2 text-gray-400" />
        ...
        <div className="space-y-3">
          {trips!.slice(0, 5).map((trip) => {
            ...
            return (
              <Link
                key={trip.id}
                href={trip.creator?.slug && trip.slug ? `/${trip.creator.slug}/${trip.slug}` : '#'}
