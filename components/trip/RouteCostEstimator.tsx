	...
	return (
		<div className={`text-sm text-gray-600 ${className}`}>
			...
			<div className="text-xs text-gray-500 mb-2">
				{t("routeCost.waypoints")
					.replace("{waypointCount}", costEstimate.waypointCount.toString())
					.replace("{requestsNeeded}", costEstimate.requestsNeeded.toString())}
			</div>

			{showSuggestions && costEstimate.suggestions.length > 0 && (
				<div className="mt-2">
					<div className="text-xs font-medium text-gray-700 mb-1">
						{t("routeCost.suggestion")}
					</div>
