...
export default function VenueDistance({
	...
	const [distanceInfo, setDistanceInfo] = useState<DistanceMatrixResult | null>(
		null,
	);
	...
	const durationText = distanceApiHelpers.formatDuration(
		distanceInfo.duration.value,
	);
	const transitUrl = buildGoogleTransitUrl(fromPlace, toPlace);
	const isLinkEnabled = Boolean(transitUrl);

	const wrapperBaseClass =
		"bg-white border-2 border-gray-300 rounded-full p-2 shadow-sm transition-colors";
