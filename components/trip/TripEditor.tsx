...
export default function TripEditor({
	...
	const publicTemplateEnabled = RestrictionProvider.hasFeature(
		userPlanId,
		RestrictionType.PUBLIC_TEMPLATE,
	);
	const canToggleTemplate =
		!disablePublishControls && (publicTemplateEnabled || formData.isTemplate);
		...
		try {
			...
			if (latestTripData) {
				...
				const updatedTrip = {
					...
					day_count: isTemplateMode
						? formData.dayCount
						: formData.dayCount || undefined,
									...
									<div>
										...
										{!formData.destinationPlace && formData.destination && (
											<p className="mt-2 text-sm text-yellow-700">
												...
												{t("tripEditor.destinationReSelectHint")}
											</p>
										)}
									</div>
								)}
								...
								{!disableDateFields && (
									<>
										<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
											<div
												className={`${isTemplateMode ? "opacity-50 pointer-events-none" : ""}`}
											>
												<label
													htmlFor="startDate"
													className="block text-sm font-medium text-gray-700 mb-2"
									...
									<div>
										...
										>
											<option value="private">
												{t("tripEditor.accessLevel.private")}
											</option>
											<option value="public">
												{t("tripEditor.accessLevel.public")}
											</option>
										</select>

										{formData.accessLevel === "public" &&
											!disablePublishControls && (
												<div className="mt-4">
													<Toggle
														label={t("trip.create.templateMode.label")}
														checked={formData.isTemplate}
														onChange={(event) =>
															handleTemplateToggle(event.target.checked)
														}
														disabled={!canToggleTemplate}
													/>
													<p
														className={`mt-1 text-xs ${publicTemplateEnabled ? "text-gray-500" : "text-red-500"}`}
													>
														{formData.isTemplate
						...
						>
							<div className="bg-white rounded-lg shadow-2xl p-6 max-w-md w-full mx-4">
								...
								<p className="text-gray-600 mb-6">
									{t("tripEditor.deleteConfirm.message").replace(
										"{title}",
										trip.title,
									)}
								</p>
								<div className="flex justify-end space-x-3">
									...
									>
										{deleting
											? t("tripEditor.deleteConfirm.deleting")
											: t("common.delete")}
									</button>
								</div>
							</div>
