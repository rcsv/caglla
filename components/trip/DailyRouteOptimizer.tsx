...
import {
	optimizeWaypoints,
	estimateRouteCost,
} from "@/lib/travel/route-optimization";
...
export default function DailyRouteOptimizer({
	...
	const handleApplyOptimization = async () => {
		...
		try {
			...
			// itineraryのIDの順序を取得
			const reorderedItineraryIds = reorderedValidItineraries.map(
				(it) => it.id,
			);
			...
			if (!response.ok) {
				...
				throw new Error(
					errorData.error || `Reorder API error: ${response.status}`,
				);
			}
			...
			const validItineraryIds = new Set(
				reorderedValidItineraries.map((it) => it.id),
			);

			// validItineraries以外の要素を元の順序で追加
			const nonValidItineraries = itineraries.filter(
				(it) => !validItineraryIds.has(it.id),
			);
			...
			setError(
				`${t("trip.routeOptimization.applyFailed")}: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	};
	...
	return (
		<div className={`space-y-4 ${className}`}>
			...
			<div className="flex items-center gap-3">
				...
				>
					{isOptimizing
						? t("trip.routeOptimization.optimizing")
						: t("trip.routeOptimization.button")}
				</PremiumButton>

				<div className="text-sm text-gray-600">
					{t("trip.routeOptimization.calculatePlaces").replace(
						"{count}",
						String(validItineraries.length),
					)}
				</div>
			</div>
				...
				<div className="space-y-4">
					...
					<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
						...
						<div className="text-sm text-yellow-700">
							{optimizationResult.optimizedOrder.map(
								(index: number, orderIndex: number) => {
									// インデックスが有効な範囲内かチェック
									if (index >= 0 && index < validItineraries.length) {
										return (
											<div key={index} className="flex items-center gap-2 mb-1">
												...
												<span>
													{validItineraries[index].place_data?.name ||
														t("routeOptimizer.unknownPlace")}
												</span>
											</div>
										);
									}
