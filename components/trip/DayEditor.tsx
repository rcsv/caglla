...
export default function DayEditor({
	...
	const handleDelete = async () => {
		...
		try {
			const response = await makeAuthenticatedRequest(
				`/api/trip/${trip.slug}/day?dayId=${day.id}`,
				{
					method: "DELETE",
				},
			);
	...
	if (isEditing) {
		return (
			<div className="space-y-2">
				...
				<p className="text-xs text-gray-400">{t("dayEditor.editHint")}</p>
			</div>
		);
	}
					...
					<div className="flex-1">
						{day.description ? (
							<div
								className={
									canEdit
										? "group cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors"
										: "p-2"
								}
							...
							>
								<p className="text-gray-400 italic">
									{itinerarySummary || t("dayEditor.placeholder")}
								</p>
							</div>
						) : (
							<div className="p-3">
								<p className="text-gray-400 italic">
									{itinerarySummary || t("dayEditor.noDescription")}
								</p>
							</div>
						)}
					</div>
