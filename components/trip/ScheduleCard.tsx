...
export default function ScheduleCard({
	...
	const [tempCostAmount, setTempCostAmount] = useState(
		itinerary.cost_amount?.toString() || "",
	);
	const [tempCostCurrency, setTempCostCurrency] = useState(
		itinerary.cost_currency || trip?.default_currency || "JPY",
	);
	...
	const { updateField, updateFields, isSaving } = useItineraryEditor(
		itinerary,
		onUpdate,
	);
	...
	const previousPlaceDataRef = useRef<PlaceData | null | undefined>(
	...
	useEffect(() => {
		...
		const hasManualTimezone =
			itinerary.timezone && itinerary.timezone !== "UTC";
		...
		const detectedTimezone = timezoneUtils.getTimezoneFromPlace(
			itinerary.place_data,
		);
		if (
			!detectedTimezone ||
	...
	useEffect(() => {
		const ensurePlaceData = async () => {
			...
			logger.debug(
				"📦 ScheduleCard: Fetching place details for missing place_data",
				{ placeId: itineraryPlaceId },
			);

			try {
				const language = getUserLanguage(user || undefined);
				const result = await placesApiHelpers.getPlaceDetails(
					itineraryPlaceId,
					language,
				);
				if (result?.place_id) {
	...
	useEffect(() => {
		const loadImage = async () => {
			if (
				...
				itinerary.place_data.photos.length > 0
			) {
				...
				const selectedPhoto =
					photos.find((p) => p.width >= targetWidth) ||
					photos.reduce((prev, curr) =>
				...
				const photoHeight =
					selectedPhoto.height || Math.round(photoWidth * 0.75);
	...
	return (
		<div className="relative overflow-visible" id={`itinerary-${itinerary.id}`}>
			...
			>
				...
				{/* カード本体 */}
				<div
					className={`flex-1 bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden ${isSelected ? "ring-2 ring-red-500 ring-opacity-50" : ""}`}
				>
					<div className="flex">
						{/* 左側: 画像（16:9アスペクト比） */}
						<ScheduleCardImage
							...
							activityIconName={
								itinerary.activity_tag
									? getSecondaryCategoryIconName(
											itinerary.activity_tag.primaryCategory,
											itinerary.activity_tag.secondaryCategory,
										)
									: undefined
							}
						>
						...
						<div className="flex-1 min-w-0 py-4 pl-4 pr-2 md:pr-3">
							...
							<div className="flex items-center space-x-2 mb-3">
								{isEditingTitle ? (
									...
									<h4
										className={
											canEdit
												? "font-semibold text-gray-900 text-xl cursor-pointer hover:text-blue-600 hover:bg-blue-50 px-2 py-1 rounded transition-colors"
												: "font-semibold text-gray-900 text-xl px-2 py-1"
										}
										onClick={
											canEdit ? () => setIsEditingTitle(true) : undefined
										}
									>
										{itinerary.title}
									</h4>
							...
							<div className="mb-4">
								{isEditingDescription ? (
									...
									<div
										onClick={
											canEdit ? () => setIsEditingDescription(true) : undefined
										}
										className={
											canEdit
												? "cursor-pointer text-sm text-gray-700 hover:bg-gray-50 p-2 rounded border border-transparent hover:border-gray-200 min-h-[2.5rem]"
												: "text-sm text-gray-700 p-2 min-h-[2.5rem]"
										}
									>
										{description ? (
											<div>
												...
												{shouldTruncate && (
													...
													>
														{isExpanded
															? t("scheduleCard.collapse")
															: t("scheduleCard.readMore")}
													</button>
												)}
											</div>
										) : (
						...
						{canEdit && (
							<ScheduleCardMenu
								...
								onDuplicateToDay={(dayId) =>
									onDuplicateToDay?.(itinerary.id, dayId)
								}
								onReservation={
									!trip.is_template
										? () => setShowReservationModal(true)
			...
			<div className="absolute bottom-0 left-1/2 transform -translate-x-1/2">
				<svg width="8" height="12" viewBox="0 0 8 12" className="text-gray-300">
					<path
						d="M4 0C1.79 0 0 1.79 0 4c0 2.21 4 8 4 8s4-5.79 4-8c0-2.21-1.79-4-4-4z"
						fill="currentColor"
					/>
				</svg>
			</div>
