	...
	return (
		<div className="flex items-center space-x-4">
			...
			<div className="flex items-center space-x-1">
				...
				{startTime || endTime ? (
					<span className={timeClass} onClick={handleTimeClick}>
						{formatTimeForDisplay(startTime)} - {formatTimeForDisplay(endTime)}
					</span>
				) : (
					<span className={timeEmptyClass} onClick={handleTimeClick}>
			...
			<div className="flex items-center space-x-1">
				...
				{costAmount !== undefined && costAmount !== null ? (
					<span className={costClass} onClick={handleCostClick}>
						{currencyUtils.formatAmount(costAmount, costCurrency || "JPY")}
					</span>
				) : (
					<span className={costEmptyClass} onClick={handleCostClick}>
					...
					>
						{hasReservation && confirmationNumber ? (
							<span className="text-xs font-mono bg-emerald-100 text-emerald-800 px-1.5 py-0.5 rounded">
								{confirmationNumber.length > 8
									? `${confirmationNumber.substring(0, 8)}...`
									: confirmationNumber}
							</span>
						) : hasReservation ? (
