...
export default function MapSearchOverlay({
	...
	const handlePlaceSelect = async (place: PlaceSearchResult) => {
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : t("placeSearch.selectFailed");
	...
	const overlayBaseClasses =
		"rounded-md border transition-all duration-200 ease-out backdrop-blur";
	...
	return (
		...
		>
			<div
				className={`${overlayBaseClasses} ${overlayVisualClasses} ${isExpanded ? "p-2" : "p-1.5"} pointer-events-auto`}
			>
				{isExpanded ? (
