	...
	useEffect(() => {
		const performOptimization = async () => {
			...
			try {
				...
				// 比較機能が有効な場合
				if (showComparison) {
					const comparison = await compareRouteOptions(
						waypoints,
						origin,
						destination,
					);
		...
		<div className={`space-y-4 ${className}`}>
			...
			<div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
				<h3 className="text-sm font-semibold text-blue-800 mb-3">
					{t("routeOptimization.optimizedRoute")}
				</h3>

				<div className="grid grid-cols-2 gap-4 mb-3">
					<div>
						<div className="text-xs text-gray-600">
							{t("routeOptimization.totalDistance")}
						</div>
					...
					<div>
						<div className="text-xs text-gray-600">
							{t("routeOptimization.totalDuration")}
						</div>
						<div className="text-sm font-semibold text-blue-700">
				...
				<div className="text-xs text-gray-600 mb-2">
					{t("routeOptimization.optimizedOrder")}:{" "}
					{optimizationResult.optimizedOrder.join(" → ")}
				</div>

				<div className="text-xs text-gray-500">
					{t("routeOptimization.apiCost")}: $
					{optimizationResult.costEstimate.estimatedCost.toFixed(3)}{" "}
					{optimizationResult.costEstimate.currency}
				</div>
			</div>
				...
				<div className="bg-green-50 border border-green-200 rounded-lg p-4">
					...
					<div className="grid grid-cols-3 gap-3 mb-3">
						...
						<div className="text-center">
							...
							<div className="text-sm font-semibold text-green-700">
								$
								{comparisonResult.cheapestRoute.costEstimate.estimatedCost.toFixed(
									3,
								)}
							</div>
						</div>
