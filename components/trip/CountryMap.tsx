		...
		const initMap = async () => {
			try {
				...
				countryGroups.forEach((group, index) => {
					...
					logger.debug(
						`Processing country: ${group.countryCode} (${group.countryNameJa})`,
						coordinate,
					);
					...
					if (coordinate) {
						...
						logger.debug(
							`Creating marker for ${group.countryNameJa} at ${coordinate.lat}, ${coordinate.lng}`,
						);
              ...
              `,
						...
						logger.warn(
							`No coordinate found for country: ${group.countryCode}`,
						);
					}
				});
	...
	return (
		<div className={`bg-white rounded-lg shadow-md p-6 ${className}`}>
			<div className="flex items-center justify-between mb-4">
				...
				<div className="text-sm text-gray-500">{countryGroups.length}カ国</div>
			</div>
