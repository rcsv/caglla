...
import {
	getZoomForPlaceTypes,
	DEFAULT_ITINERARY_ZOOM,
} from "@/lib/travel/map-zoom";
...
export default function TripMap({
	...
	const initialCenterLat =
		typeof initialCenter?.lat === "function"
			? initialCenter.lat()
			: (initialCenter?.lat ?? null);
	const initialCenterLng =
		typeof initialCenter?.lng === "function"
			? initialCenter.lng()
			: (initialCenter?.lng ?? null);
		...
		const initializeMap = async () => {
			...
			try {
				...
				newMap.addListener("click", async (event: any) => {
					...
					if (event.placeId) {
						console.log(
							"🟡 [MAP CLICK → POI] Google POI detected via click event",
						);
						...
						try {
							...
							if (response.ok) {
								...
								if (data.status === "OK" && data.result) {
									...
									console.log(
										"🟢 [MAP CLICK → POI] Setting POI data with name:",
										newPoiData,
									);
									logger.debug(
										"🟡 Google POI marker clicked via click event:",
										newPoiData,
									);
									setInternalPoiData(newPoiData);
									onPoiDataUpdateRef.current?.(newPoiData);
									...
									console.warn(
										"⚠️ [MAP CLICK → POI] Failed to get place details, using fallback",
									);
									setInternalPoiData(newPoiData);
									onPoiDataUpdateRef.current?.(newPoiData);
								}
							...
							console.error(
								"❌ [MAP CLICK → POI] Error fetching place details:",
								error,
							);
							// エラー時のフォールバック
							const newPoiData = {
				...
				const poiClickListener = newMap.addListener(
					"poi_click",
					(event: any) => {
	...
	useEffect(() => {
		...
		const newMarkers = validItineraries.map((itinerary, index) => {
			...
			marker.addListener("click", () => {
				...
				if (itinerary.place_data?.place_id) {
					...
					console.warn(
						"⚠️ [ITINERARY MARKER] No place_id found:",
						itinerary.title,
					);
					logger.warn(
						"⚠️ Itinerary marker clicked but no place_id found:",
						itinerary.title,
					);
				}
		...
		if (validItineraries.length >= 2) {
			...
			const request = {
				...
				destination: {
					lat: validItineraries[validItineraries.length - 1].place_data!
						.geometry!.location.lat,
					lng: validItineraries[validItineraries.length - 1].place_data!
	...
	useEffect(() => {
		...
		if (
			selectedPlaceId &&
			activePoi.placeId &&
			activePoi.placeId === selectedPlaceId
		) {
			return () => {
				cleanupMarker();
			};
		...
		const positionSource =
			activePoi.placeData?.geometry?.location || activePoi.location;
	...
	useEffect(() => {
		...
		const zoom = getZoomForPlaceTypes(selectedItinerary.place_data?.types);
		logger.debug(
			"🎯 TripMap: Focusing on selected itinerary with types-based zoom",
			{
