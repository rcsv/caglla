...
import { aggregateCostsWithDetails, CostSummaryWithDetails, formatMultipleCostSummaries } from '@/lib/travel/cost-aggregation'
...
export default function TripCostDisplay({
  ...
  if (!costSummary.hasCosts) {
    return (
      <Card title={<div className="flex items-center"><MoneyIcon className="w-5 h-5 mr-2" color="#16a34a" />{t('cost.title')}</div>} className={className}>
            ...
            <div className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-md">
              ...
              <div className="flex items-center gap-2">
                ...
                {cost.items && cost.items.length > 0 && (
                  ...
                  >
                    {expandedCurrencies.has(cost.currency) ? t('cost.collapse') : t('cost.viewDetails')}
                  </button>
                )}
              </div>
            </div>
        ...
        {costSummary.totalCosts.length > 1 && (
          <div className="border-t border-gray-200 pt-3 mt-3">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-600">{t('cost.total')}</span>
