...
function LimitBar({ label, checkResult, className = "" }: LimitBarProps) {
	...
	return (
		<div className={`space-y-2 ${className}`}>
			...
			<p className="text-xs text-gray-500">{checkResult.message}</p>
		</div>
	);
}
	...
	return (
		<div className={`bg-white rounded-lg border p-6 ${className}`}>
			<div className="flex items-center justify-between mb-4">
				<h3 className="text-lg font-semibold text-gray-900">プラン制限状況</h3>
				<span
			...
			<div className="space-y-4">
				<LimitBar label="旅行データ数" checkResult={checks.travelCount} />

				<LimitBar label="旅行日数" checkResult={checks.travelDays} />

				<LimitBar label="ストレージ容量" checkResult={checks.storage} />

				<LimitBar label="写真アップロード数" checkResult={checks.photos} />
			...
			{upgradeMessage && !hasAnyLimitExceeded && (
				<div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
					...
					<p className="text-xs text-yellow-600 mt-1">{upgradeMessage}</p>
				</div>
			)}
...
export function usePlanLimits(plan: SubscriptionPlan, usage: UsageStats) {
	...
	const hasAnyLimitExceeded = checks.hasAnyLimitExceeded;
	const upgradeMessage = PlanLimitChecker.generateUpgradeMessage(plan, usage);
	const limitExceededMessage = PlanLimitChecker.generateLimitExceededMessage(
		plan,
		usage,
	);
