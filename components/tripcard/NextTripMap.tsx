...
export default function NextTripMap({
	trip,
	className = "",
}: NextTripMapProps) {
	...
	useEffect(() => {
		const initializeMap = async () => {
			try {
				...
				logger.error(
					"NextTripMap: Google Maps APIの読み込みに失敗しました:",
					error,
				);
				setError(
	...
	// マーカーを作成・更新する関数
	const updateMapAndMarker = useCallback(
	...
	useEffect(() => {
		...
		logger.debug(
			"NextTripMap: 旅行データの座標を使用（現在地のフォールバック無効化）",
			center,
		);
	...
	return (
		<div className={`relative h-full ${className}`}>
			{loading && (
				<div
					className={`absolute inset-0 bg-gray-100 flex items-center justify-center ${getZIndexClass("MAIN_CONTENT")}`}
				>
					<div className="text-center">
						<div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mx-auto mb-2"></div>
						<p className="text-sm text-gray-600">
							{require("@/lib/i18n").t("loading.mapLoading")}
						</p>
					</div>
				</div>
			)}
			...
			{/* 旅行情報のオーバーレイ */}
			<div
				className={`absolute top-4 left-4 bg-white rounded-lg shadow-xl border border-gray-200 p-3 max-w-xs ${getZIndexClass("MAIN_CONTENT")}`}
			>
				<div className="text-sm text-gray-600">
					<div className="font-medium text-gray-900 mb-1">{trip.title}</div>
					<div>
						{trip.destination && (
