...
export const TripStatsRow: React.FC<TripStatsRowProps> = ({
  ...
  if (!shouldShow.days && !shouldShow.venues && !shouldShow.photos && !shouldShow.checklists) {
    return null
  }

  return (
    <div className={`mt-1 flex flex-wrap gap-3 text-[11px] text-slate-500 ${className}`}>
      {shouldShow.days && (
        <span className="inline-flex items-center gap-1">
          <Icon icon="mdi:calendar-range" className="h-3.5 w-3.5 text-slate-400" />
          <span>{days} days</span>
        </span>
      )}
      {shouldShow.venues && (
        <span className="inline-flex items-center gap-1">
          <Icon icon="mdi:map-marker-radius" className="h-3.5 w-3.5 text-slate-400" />
          <span>{venues} venues</span>
        </span>
      )}
      {shouldShow.photos && (
        <span className="inline-flex items-center gap-1">
          <Icon icon="mdi:image-multiple" className="h-3.5 w-3.5 text-slate-400" />
          <span>{photos}</span>
        </span>
      )}
      {shouldShow.checklists && (
        <span className="inline-flex items-center gap-1">
          <Icon icon="mdi:check-decagram" className="h-3.5 w-3.5 text-slate-400" />
