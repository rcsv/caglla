...
export const TripCard: React.FC<TripCardProps> = ({
	...
	const likesCount =
		trip.social_stats?.likes_count ??
		(typeof trip.likes_count === "number" ? trip.likes_count : 0);
	...
	const LikeBadge = ({ size = "sm" }: { size?: "sm" | "md" }) => {
		...
		return (
			<span
				className={baseClass}
				aria-label={t("trip.likes.count", { count: likesCount })}
			>
	...
	const SocialStats = ({
		...
		const hasStats =
			likesCount > 0 || (showComments && commentsCount > 0) || sharesCount > 0;
	...
	const accessLevel =
		trip.access_level === "public" || trip.access_level === "private"
			? trip.access_level
			: "private";
					...
					<div className="relative h-full px-4 py-3 md:px-5 md:py-4 flex flex-col gap-1.5 md:gap-2 text-white">
							...
							{trip.start_date && trip.end_date && (
								<span className="flex items-center gap-1">
									...
									{(() => {
										...
										const { futureTrips, pastTrips } =
											dateUtils.sortTripsByDate([trip]);
										if (futureTrips.length > 0) {
											return dateUtils.formatFutureTripDate(
												trip.start_date,
												trip.end_date,
												language,
											);
										} else if (pastTrips.length > 0) {
											return dateUtils.formatPastTripDate(
												trip.start_date,
												trip.end_date,
												language,
											);
						...
						<div className="mt-2">
							<div className="flex items-center gap-3">
								{likesCount > 0 && (
									<span
										className="inline-flex items-center gap-1 text-white/85"
										aria-label={t("trip.likes.count", { count: likesCount })}
									>
										<Icon
											icon="mdi:heart"
			...
			<Link href={getTripUrl()} className="block group">
				...
				>
					...
					<div className="absolute inset-x-0 bottom-0 p-6 text-white">
						<h3 className="text-3xl md:text-4xl font-bold drop-shadow-md line-clamp-2">
							{trip.title}
						</h3>
						{trip.description && (
	...
	return (
		<Link href={getTripUrl()} className="block group">
			<Card interactive padding="md" className={`h-full ${accentBorderClass}`}>
				{trip.image_url && (
					<div className="mb-4">
						<div
							className={`relative w-full h-32 rounded-lg overflow-hidden ${isPastTrip ? "shadow-inner-burned" : ""}`}
						>
							<Image
								src={trip.image_url}
								alt={trip.title}
							...
							{isPastTrip && (
								<div
									...
									style={{
										boxShadow:
											"inset 0 0 20px rgba(139, 69, 19, 0.3), inset 0 0 40px rgba(160, 82, 45, 0.2), inset 0 0 60px rgba(139, 69, 19, 0.1)",
									}}
								/>
							)}
						</div>
				...
				<div className="flex items-center justify-between gap-4">
					{trip.start_date && trip.end_date && (
						<p className="text-gray-500 text-sm flex items-center gap-1">
							...
							{(() => {
								const language = getUserLanguage();
								const { futureTrips, pastTrips } = dateUtils.sortTripsByDate([
									trip,
								]);
								if (futureTrips.length > 0) {
									return dateUtils.formatFutureTripDate(
