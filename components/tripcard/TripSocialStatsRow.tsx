...
const SocialStatItem: React.FC<SocialStatItemProps> = ({ icon, value, className }) => {
  return (
...
export const TripSocialStatsRow: React.FC<TripSocialStatsRowProps> = ({
  ...
  return (
    <div className={`flex items-center gap-2 text-[11px] text-slate-500 ${className}`}>
      ...
      {showComments && <SocialStatItem icon="mdi:message-outline" value={comments} />}
      {showShares && <SocialStatItem icon="mdi:share-outline" value={shares} />}
      {showReplicas && <SocialStatItem icon="mdi:content-copy" value={replicas} />}
    </div>
  )
}
